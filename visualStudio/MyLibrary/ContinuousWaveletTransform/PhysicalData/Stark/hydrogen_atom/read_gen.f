...
c$$$        dimension dlambda(id_maxi,10,100)
...
c$$$        read(5,*) TEMP ! temperature in
c$$$        read(5,*) DENS ! electronic density in cm-3
        TEMP = 100000.0
        DENS = 1d17
...
c$$$          pr0(:,:)=0.
        ...
        write(73,*) '# Te ne dalfa f0 fainu intensity'
...
c$$$             dlambda(id,j,i) = f00(id)*dalfa
             domega=-cspeed_pi*dlambda/((ambda+dlambda)*ambda) !rd/s
c$$$             domega=-cspeed_pi*dlambda(id,j,i)/
c$$$     $            ((ambda+dlambda(id,j,i))*ambda)  !rd/s
...
!     s          dom(id,j,i),olines(id,j,i),oline(id,j,i)
             write(73,1941) Tempe(j),dense(id),
     s          dalfa,f00(id),fainu,o1lines(id,j,i),ambda
           end do
         end do
c$$$ 1941    format(5(1x,1pe10.3))
 1941    format(6(1x,1pe10.3),1x,0pf8.2)
!         write(73,*)

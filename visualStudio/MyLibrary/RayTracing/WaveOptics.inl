...
bool PointOnSurface::Go1Dim(PointOnSurface const* obj, const size_t num, const double lambda){
	A = 0.0;	
	for(size_t i=0; i<num; ++i){
		long double Lij = 0.0;
		for(size_t j=0; j<3; ++j)
			Lij += sqrt((obj[i].x[j] - x[j])*(obj[i].x[j] - x[j]));
		
		long double phi_i = (twopi * Lij/lambda); 
		phi_i = phi_i - floor(phi_i/twopi)*twopi;
		A += obj[i].A * exp_euler(phi_i)/Lij;
	}
	return true;
}

bool PointOnSurface::Go(PointOnSurface const* obj, const size_t num, const double lambda){
	A = 0.0;	
	for(size_t i=0; i<num; ++i){
		long double Lij = 0.0;
		for(size_t j=0; j<3; ++j)
		...
		long double phi_i = (twopi * Lij/lambda); 
		phi_i = phi_i - floor(phi_i/twopi)*twopi;
		A += obj[i].A * exp_euler(phi_i)/(Lij*Lij);
	}
	return true;
}

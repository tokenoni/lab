...
#ifndef CCGSL_BLOCK_LONG_DOUBLE_HPP
#define CCGSL_BLOCK_LONG_DOUBLE_HPP
...
namespace gsl {
  /**
   * This class handles vector_long_doubles as shared handles. It models a random access container
   * so that STL functions work with block_long_double.
   */
  ...
  public:
    ...
    // Refines assignable
    /**
     * The default constructor creates a new block_long_double with n elements
     * @param n The number of elements in the block_long_double
     */
    explicit block_long_double( size_t const n ){
      ccgsl_pointer = gsl_block_long_double_alloc( n );
      // just plausibly we could allocate block_long_double but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_block_long_double_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_block_long_double. This is not usually a good idea. In this case
     * we should not use gsl_block_long_double_free() to deallocate the memory.
     * @param v The block_long_double
     */
    explicit block_long_double( gsl_block_long_double* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the block_long_double. Use clone() if you want a full copy.
     * @param v The block_long_double to copy.
     */
    block_long_double( block_long_double const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
     ...
     * @param v The block_long_double to copy
     */
    block_long_double& operator=( block_long_double const& v ){
      // first, possibly delete anything pointed to by this
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_long_double_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
     ...
     */
    block_long_double clone() const {
      block_long_double copy( ccgsl_pointer->size );
      // Now copy
      memcpy( copy.ccgsl_pointer->data, ccgsl_pointer->data, ccgsl_pointer->size * sizeof( long double ) );
      // return new object
      return copy;
    }
    // destructor
    ...
    // == operator
    /**
     * Two block_long_double objects are identically equal if their elements are identical.
     * @param v The block_long_double to be compared with @c this
     * @return @c true or @c false according as @c this and @c v have
     * identical elements or not
     */
    bool operator==( block_long_double const& v ) const {
      // trivially equal if gsl_block_long_double*s are identical
    ...
    // != operator
    /**
     * Two block_long_double objects are different equal if their elements are not identical.
     * @param v The block_long_double to be compared with @c this
     * @return @c false or @c true according as @c this and @c v have
     * identical elements or not
     */
    bool operator!=( block_long_double const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a block_long_double is nonnegative.
     * @param v The block_long_double to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( block_long_double const& v ) const {
      // null block_long_double comes first
  ...
  private:
    ...
    class iterator_base
      ...
      friend class block_long_double;
    public:
      // // type iterator_traits<block_long_double>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_long_double not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
  	...
  	// Warn if iterators do not point to same block_long_double
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different block_long_doubles", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same block_long_double and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a block_long_double
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
      ...
      /**
       * This constructor allows block_long_double to create non-default iterators.
       * @param v The block_long_double that creates @c this
      ...
      /**
       * Store a pointer to a block_long_double we can iterate over: 0 if no block_long_double.
       */
      container* v;
      /**
       * Mark position of iterator within block_long_double.
       */
     ...
     */
    template<bool reverse> class iterator_t : public iterator_base<block_long_double,long double,reverse>{
    public:
  	...
  	iterator_base<block_long_double,long double,reverse>::v = i.v;
  	iterator_base<block_long_double,long double,reverse>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<block_long_double,long double,reverse>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse>& operator--(){
  	iterator_base<block_long_double,long double,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_long_double,long double,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<block_long_double,long double,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse>() : iterator_base<block_long_double,long double,reverse>(){}
    protected:
      friend class block_long_double;
      // We need a constructor for block_long_double
      /**
       * This constructor allows block_long_double to create non-default iterators.
       * @param v The block_long_double that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse>( block_long_double* v, difference_type position )
      : iterator_base<block_long_double,long double,reverse>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse> class const_iterator_t
      : public iterator_base<block_long_double const,long double,reverse>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<block_long_double const,long double,reverse>::v = i.v;
  	iterator_base<block_long_double const,long double,reverse>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<block_long_double const,long double,reverse>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse>& operator--(){
  	iterator_base<block_long_double const,long double,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_long_double const,long double,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse> const& i ) const {
  	return iterator_base<block_long_double const,long double,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse>() : iterator_base<block_long_double,long double,reverse>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse>( block_long_double const* v, difference_type position )
      : iterator_base<block_long_double const,long double,reverse>( v, position ){}
    };
  public:
    /**
    ...
    // begin()
    /**
     * Get iterator pointing to first block_long_double element.
     * @return iterator pointing to first block_long_double element
     */
    iterator begin(){
    ...
    // end()
    /**
     * Get iterator pointing beyond last block_long_double element.
     * @return iterator pointing beyond last block_long_double element
     */
    iterator end(){
    ...
    // size()
    /**
     * The size (number of elements) of the block_long_double.
     * @return The size of the block_long_double
     */
    size_type size() const { return ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the block_long_double. Identical to
    ...
    // empty()
    /**
     * Find if the block_long_double is empty.
    ...
    /**
     * Swap two block_long_double objects. This works even if the block_long_double objects have
     * different sizes because it swaps pointers.
     * @param v The block_long_double to swap with @c this.
     */
    void swap( block_long_double& v ){
      gsl_block_long_double* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last block_long_double element.
     * @return iterator pointing beyond last block_long_double element
     */
    ...
    long double& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_long_double is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_long_double", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
  ...
  private:
     ...
     * The shared pointer
     */
    gsl_block_long_double* ccgsl_pointer;
    /**
  ...
  public:
    ...
    /**
     * Get the gsl_block_long_double.
     * @return the gsl_block_long_double
     */
    gsl_block_long_double* get() const { return ccgsl_pointer; }
    /**
     * Find if this is the only object sharing the gsl_block_long_double.
     * @return @c true or @c falses according as 
     * this is the only block_long_double object sharing the gsl_block_long_double
     */
    bool unique() const { return *count == 1; }
    /**
     * Find how many block_long_double objects share this pointer.
     * @return the number of block_long_double objects that share this pointer
     */
    size_t use_count() const { return *count; }
    /**
     * Allow conversion to bool.
     * @return @c true or @c false according as this contains a pointer
     * to a gsl_block_long_double
     */
   ...
   * @see block_long_double::iterator::operator+()
   */
  inline block_long_double::iterator operator+
  ( block_long_double::iterator::difference_type const n,
  			      block_long_double::iterator const& i ){
    return i + n;
  }
   ...
   * @see block_long_double::iterator::operator+()
   */
  inline block_long_double::const_iterator operator+
  ( block_long_double::const_iterator::difference_type const n,
  				    block_long_double::const_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_long_double::iterator::operator+()
   */
  inline block_long_double::reverse_iterator operator+
  ( block_long_double::reverse_iterator::difference_type const n,
  			      block_long_double::reverse_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_long_double::iterator::operator+()
   */
  inline block_long_double::const_reverse_iterator operator+
  ( block_long_double::const_reverse_iterator::difference_type const n,
  			      block_long_double::const_reverse_iterator const& i ){
    return i + n;
  }

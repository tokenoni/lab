...
namespace gsl {
  ...
  public:
     ...
     * we should not use gsl_block_free() to deallocate the memory.
     * @param v The block
     */
    ...
    block& operator=( block const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
  ...
  public:
     ...
     */
    void swap( block& v ){
      gsl_block* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    double& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }

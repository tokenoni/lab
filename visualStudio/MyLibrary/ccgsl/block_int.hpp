...
namespace gsl {
  ...
  public:
	...
	gsl_block_int_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
     ...
     * we should not use gsl_block_int_free() to deallocate the memory.
    ...
    block_int& operator=( block_int const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_int_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
  ...
  public:
    ...
    void swap( block_int& v ){
      gsl_block_int* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    int& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_int is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_int", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }

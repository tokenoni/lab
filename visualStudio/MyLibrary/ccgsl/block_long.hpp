...
#ifndef CCGSL_BLOCK_LONG_HPP
#define CCGSL_BLOCK_LONG_HPP
...
namespace gsl {
  /**
   * This class handles vector_longs as shared handles. It models a random access container
   * so that STL functions work with block_long.
   */
  class block_long {
  public:
    /**
    ...
    // Refines assignable
    /**
     * The default constructor creates a new block_long with n elements
     * @param n The number of elements in the block_long
     */
    explicit block_long( size_t const n ){
      ccgsl_pointer = gsl_block_long_alloc( n );
      // just plausibly we could allocate block_long but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_block_long_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_block_long. This is not usually a good idea. In this case
     * we should not use gsl_block_long_free() to deallocate the memory.
     * @param v The block_long
     */
    explicit block_long( gsl_block_long* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the block_long. Use clone() if you want a full copy.
     * @param v The block_long to copy.
     */
    block_long( block_long const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
     ...
     * @param v The block_long to copy
     */
    block_long& operator=( block_long const& v ){
      // first, possibly delete anything pointed to by this
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_long_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
     ...
     */
    block_long clone() const {
      block_long copy( ccgsl_pointer->size );
      // Now copy
      memcpy( copy.ccgsl_pointer->data, ccgsl_pointer->data, ccgsl_pointer->size * sizeof( long ) );
      // return new object
      return copy;
    }
    // destructor
    ...
    // == operator
    /**
     * Two block_long objects are identically equal if their elements are identical.
     * @param v The block_long to be compared with @c this
     * @return @c true or @c false according as @c this and @c v have
     * identical elements or not
     */
    bool operator==( block_long const& v ) const {
      // trivially equal if gsl_block_long*s are identical
    ...
    // != operator
    /**
     * Two block_long objects are different equal if their elements are not identical.
     * @param v The block_long to be compared with @c this
     * @return @c false or @c true according as @c this and @c v have
     * identical elements or not
     */
    bool operator!=( block_long const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a block_long is nonnegative.
     * @param v The block_long to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( block_long const& v ) const {
      // null block_long comes first
	...
	long const t = ccgsl_pointer->data[i];
	long const u =v.ccgsl_pointer->data[i];
	if( t < u ) return true;
	if( u < t ) return false;
      }
      // elements match.
  ...
  private:
    ...
    class iterator_base
      ...
      friend class block_long;
    public:
      // // type iterator_traits<block_long>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_long not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
  	...
  	// Warn if iterators do not point to same block_long
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different block_longs", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same block_long and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a block_long
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
      ...
      /**
       * This constructor allows block_long to create non-default iterators.
       * @param v The block_long that creates @c this
      ...
      /**
       * Store a pointer to a block_long we can iterate over: 0 if no block_long.
       */
      container* v;
      /**
       * Mark position of iterator within block_long.
       */
     ...
     */
    template<bool reverse> class iterator_t : public iterator_base<block_long,long,reverse>{
    public:
  	...
  	iterator_base<block_long,long,reverse>::v = i.v;
  	iterator_base<block_long,long,reverse>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<block_long,long,reverse>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse>& operator--(){
  	iterator_base<block_long,long,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_long,long,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<block_long,long,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse>() : iterator_base<block_long,long,reverse>(){}
    protected:
      friend class block_long;
      // We need a constructor for block_long
      /**
       * This constructor allows block_long to create non-default iterators.
       * @param v The block_long that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse>( block_long* v, difference_type position )
      : iterator_base<block_long,long,reverse>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse> class const_iterator_t
      : public iterator_base<block_long const,long,reverse>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<block_long const,long,reverse>::v = i.v;
  	iterator_base<block_long const,long,reverse>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<block_long const,long,reverse>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse>& operator--(){
  	iterator_base<block_long const,long,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_long const,long,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse> const& i ) const {
  	return iterator_base<block_long const,long,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse>() : iterator_base<block_long,long,reverse>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse>( block_long const* v, difference_type position )
      : iterator_base<block_long const,long,reverse>( v, position ){}
    };
  public:
    /**
    ...
    // begin()
    /**
     * Get iterator pointing to first block_long element.
     * @return iterator pointing to first block_long element
     */
    iterator begin(){
    ...
    // end()
    /**
     * Get iterator pointing beyond last block_long element.
     * @return iterator pointing beyond last block_long element
     */
    iterator end(){
    ...
    // size()
    /**
     * The size (number of elements) of the block_long.
     * @return The size of the block_long
     */
    size_type size() const { return ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the block_long. Identical to
    ...
    // empty()
    /**
     * Find if the block_long is empty.
    ...
    /**
     * Swap two block_long objects. This works even if the block_long objects have
     * different sizes because it swaps pointers.
     * @param v The block_long to swap with @c this.
     */
    void swap( block_long& v ){
      gsl_block_long* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last block_long element.
     * @return iterator pointing beyond last block_long element
     */
    ...
    long& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_long is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_long", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
  ...
  public:
    ...
    /**
     * Get the gsl_block_long.
     * @return the gsl_block_long
     */
    gsl_block_long* get() const { return ccgsl_pointer; }
    /**
     * Find if this is the only object sharing the gsl_block_long.
     * @return @c true or @c falses according as 
     * this is the only block_long object sharing the gsl_block_long
     */
    bool unique() const { return *count == 1; }
    /**
     * Find how many block_long objects share this pointer.
     * @return the number of block_long objects that share this pointer
     */
   ...
   * @see block_long::iterator::operator+()
   */
  inline block_long::iterator operator+
  ( block_long::iterator::difference_type const n,
   ...
   * @see block_long::iterator::operator+()
   */
  inline block_long::const_iterator operator+
  ( block_long::const_iterator::difference_type const n,
  				    block_long::const_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_long::iterator::operator+()
   */
  inline block_long::reverse_iterator operator+
  ( block_long::reverse_iterator::difference_type const n,
  			      block_long::reverse_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_long::iterator::operator+()
   */
  inline block_long::const_reverse_iterator operator+
  ( block_long::const_reverse_iterator::difference_type const n,
  			      block_long::const_reverse_iterator const& i ){
    return i + n;
  }

...
#ifndef CCGSL_VECTOR_USHORT_HPP
#define CCGSL_VECTOR_USHORT_HPP
...
namespace gsl {
  // declare matrix_ushort class
  class matrix_ushort;
  /**
   * This class handles vector_ushort objects as shared handles. It models a random access container
   * so that STL functions work with vector_ushort.
   *
  ...
  public:
     ...
     * @param n The number of elements in the vector_ushort
     */
    explicit vector_ushort( size_t const n ){
      ccgsl_pointer = gsl_vector_ushort_alloc( n );
      // just plausibly we could allocate vector_ushort but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_ushort_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_vector_ushort. This is not usually a good idea. In this case
     * we should not use gsl_vector_ushort_free() to deallocate the memory.
     * @param v The vector_ushort
     */
    explicit vector_ushort( gsl_vector_ushort* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the vector_ushort. Use clone() if you want a full copy.
     * @param v The vector_ushort to copy.
     */
    vector_ushort( vector_ushort const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      ++*count; // vector_ushort is now shared.
    }
    // assignment operator
    /**
     ...
     * @param v The vector_ushort to copy
     */
    vector_ushort& operator=( vector_ushort const& v ){
      // first, possibly delete anything pointed to by @c this
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_vector_ushort_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
      ccgsl_pointer = v.ccgsl_pointer;
      count = v.count;
      ++*count; // block_ushort is now shared.
      return *this;
    }
    // clone()
    /**
    ...
    vector_ushort clone() const {
      vector_ushort copy( get()->size );
      // Now copy
      gsl_vector_ushort_memcpy( copy.get(), get() );
      // return new object
      return copy;
    }
    // destructor
     ...
     * @param v The vector_ushort to be compared with @c this
    ...
    bool operator==( vector_ushort const& v ) const {
      // trivially equal if gsl_vector_ushort*s are identical
  	...
  	if( gsl_vector_ushort_get( ccgsl_pointer, i ) != gsl_vector_ushort_get( v.ccgsl_pointer, i ) ) return false; 
      return true;
    }
    // != operator
    /**
     ...
     * identical elements or not
     */
    bool operator!=( vector_ushort const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a vector_ushort is nonnegative.
     * @param v The vector_ushort to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( vector_ushort const& v ) const {
      // null vector_ushort comes first
	...
	unsigned short const t = gsl_vector_ushort_get( ccgsl_pointer, i );
	unsigned short const u =gsl_vector_ushort_get( v.ccgsl_pointer, i );
	if( t < u ) return true;
	if( u < t ) return false;
      }
      // elements match.
  ...
  private:
    ...
    public:
      // // type iterator_traits<vector_ushort>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "vector_ushort not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
  	...
  	// Warn if iterators do not point to same vector_ushort
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different vector_ushort objects", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same vector_ushort and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a vector_ushort
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
      ...
      /**
       * Store a pointer to a vector_ushort we can iterate over: 0 if no vector_ushort.
       */
      container* v;
      /**
       * Mark position of iterator within vector_ushort.
       */
     ...
     */
    template<bool reverse_t> class iterator_t : public iterator_base<vector_ushort,unsigned short,reverse_t>{
    public:
  	...
  	iterator_base<vector_ushort,unsigned short,reverse_t>::v = i.v;
  	iterator_base<vector_ushort,unsigned short,reverse_t>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<vector_ushort,unsigned short,reverse_t>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse_t>& operator--(){
  	iterator_base<vector_ushort,unsigned short,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<vector_ushort,unsigned short,reverse_t>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<vector_ushort,unsigned short,reverse_t>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse_t>() : iterator_base<vector_ushort,unsigned short,reverse_t>(){}
    protected:
      friend class vector_ushort;
      // We need a constructor for vector_ushort
      /**
       * This constructor allows vector_ushort to create non-default iterators.
       * @param v The vector_ushort that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse_t>( vector_ushort* v, difference_type position )
      : iterator_base<vector_ushort,unsigned short,reverse_t>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse_t> class const_iterator_t
      : public iterator_base<vector_ushort const,unsigned short,reverse_t>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<vector_ushort const,unsigned short,reverse_t>::v = i.v;
  	iterator_base<vector_ushort const,unsigned short,reverse_t>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<vector_ushort const,unsigned short,reverse_t>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse_t>& operator--(){
  	iterator_base<vector_ushort const,unsigned short,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<vector_ushort const,unsigned short,reverse_t>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse_t> const& i ) const {
  	return iterator_base<vector_ushort const,unsigned short,reverse_t>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse_t>() : iterator_base<vector_ushort,unsigned short,reverse_t>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse_t>( vector_ushort const* v, difference_type position )
      : iterator_base<vector_ushort const,unsigned short,reverse_t>( v, position ){}
    };
  public:
    /**
     ...
     * @return The size of the vector_ushort
     */
    size_type size() const { return ccgsl_pointer == 0 ? 0 : ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the vector_ushort. Identical to
    ...
    /**
     * Swap two vector_ushort objects. This works even if the vector_ushort objects have different sizes
     * because it swaps pointers.
     * @param v The vector_ushort to swap with @c this.
     */
    void swap( vector_ushort& v ){
      gsl_vector_ushort* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last vector_ushort element.
     * @return iterator pointing beyond last vector_ushort element
     */
     ...
     * @return a reference to a unsigned short
     */
    unsigned short& operator[]( size_t const n ){
      // Always try to return something
      static unsigned short something = 0;
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_ushort is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_ushort", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
  ...
  public:
    ...
    gsl_vector_ushort const* get() const {
      ...
      return ccgsl_pointer; }
    /**
     * Find if @c this is the only object sharing the gsl_vector_ushort.
     * @return @c true or @c falses according as 
     * this is the only vector_ushort object sharing the gsl_vector_ushort
     */
    bool unique() const { return *count == 1; }
    /**
     * Find how many vector_ushort objects share this pointer.
     * @return the number of vector_ushort objects that share this pointer
     */
    size_t use_count() const { return *count; }
    /**
     * Allow conversion to bool.
     * @return @c true or @c false according as @c this contains a pointer
     * to a gsl_vector_ushort
     */
    operator bool() const { return ccgsl_pointer != 0; }

    // GSL functions
    /**
     * C++ version of gsl_vector_ushort_calloc(). This constructs a vector_ushort object with entries
     * initialised to zero.
     * @param n The size of the vector_ushort
     * @return A vector_ushort initialised to zero
     */
    static vector_ushort calloc( size_t const n ){ return vector_ushort( gsl_vector_ushort_calloc( n ) ); }
    /**
     * C++ version of gsl_vector_ushort_set_zero().
     */
    void set_zero(){ gsl_vector_ushort_set_zero( get() ); }
    /**
     * C++ version of gsl_vector_ushort_set_all().
     * @param x The value to which all elements are set
     */
    void set_all( unsigned short x ){ gsl_vector_ushort_set_all( get(), x ); }
    /**
     * C++ version of gsl_vector_ushort_set_basis(). Creates a basis vector_ushort with one nonzero element.
     * @param i The element to be set to 1.
     * @return error code on failure
     */
    int set_basis( size_t i ){ return gsl_vector_ushort_set_basis( get(), i ); }
    /**
     * C++ version of gsl_vector_ushort_memcpy().
     * @param src source vector_ushort
     * @return error code on failure
     */
    int memcpy( vector_ushort const& src ){ return gsl_vector_ushort_memcpy( get(), src.get() ); }
    /**
     * C++ version of gsl_vector_ushort_reverse().
     * @return error code on failure
     */
    int reverse(){ return gsl_vector_ushort_reverse( get() ); }
    /**
    ...
    int swap_elements( size_t const i, size_t const j ){
      return gsl_vector_ushort_swap_elements( get(), i, j ); }
    /**
     * C++ version of gsl_vector_ushort_max().
     * @return maximum element of vector_ushort
     */
    unsigned short max() const { return gsl_vector_ushort_max( get() ); }
    /**
     * C++ version of gsl_vector_ushort_min().
     * @return minimum element of vector_ushort
     */
    unsigned short min() const { return gsl_vector_ushort_min( get() ); }
    /**
     * C++ version of gsl_vector_ushort_minmax().
     * @param min_out minimum element of vector_ushort
     * @param max_out maximum element of vector_ushort
     */
    void minmax( unsigned short* min_out, unsigned short* max_out ) const {
      gsl_vector_ushort_minmax( get(), min_out, max_out ); }
    /**
     * C++ version of gsl_vector_ushort_max_index().
     * @return index of maximum value of vector_ushort
     */
    size_t max_index() const { return gsl_vector_ushort_max_index( get() ); }
    /**
     * C++ version of gsl_vector_ushort_min_index().
     * @return index of minimum value of vector_ushort
     */
    size_t min_index() const { return gsl_vector_ushort_min_index( get() ); }
    /**
     * C++ version of gsl_vector_ushort_minmax_index().
     * @param imin index of minimum value of vector_ushort
     * @param imax index of maximum value of vector_ushort
     */
    void minmax_index( size_t* imin, size_t* imax ) const {
      gsl_vector_ushort_minmax_index( get(), imin, imax ); }
    /**
     * C++ version of gsl_vector_ushort_add().
     * @param b vector_ushort to add to @c this
     * @return error code on failure
     */
    int add( vector_ushort const& b ){ return gsl_vector_ushort_add( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_ushort_sub().
     * @param b vector_ushort to subtract from @c this
     * @return error code on failure
     */
    int sub( vector_ushort const& b ){ return gsl_vector_ushort_sub( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_ushort_mul().
     * @param b vector_ushort to multiply elementwise with @c this
     * @return error code on failure
     */
    int mul( vector_ushort const& b ){ return gsl_vector_ushort_mul( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_ushort_div().
     * @param b vector_ushort to dividev @c this by elementwise
     * @return error code on failure
     */
    int div( vector_ushort const& b ){ return gsl_vector_ushort_div( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_ushort_scale().
     * @param x constant to multiply @c this by
     * @return error code on failure
     */
    int scale( unsigned short const x ){ return gsl_vector_ushort_scale( get(), x ); }
    /**
     * C++ version of gsl_vector_ushort_add_constant().
     * @param x constant to add to each element of @c this
     * @return error code on failure
     */
    int add_constant( unsigned short const x ){ return gsl_vector_ushort_add_constant( get(), x ); }
    /**
     * C++ version of gsl_vector_ushort_isnull().
     * @return @c +1 or @c 0 according as elements are all zero or not
     */
    int isnull() const { return gsl_vector_ushort_isnull( get() ); }
    /**
     * C++ version of gsl_vector_ushort_ispos().
     * @return @c +1 or @c 0 according as elements are all positive or not
     */
    int ispos() const { return gsl_vector_ushort_ispos( get() ); }
    /**
     * C++ version of gsl_vector_ushort_isneg().
     * @return @c +1 or @c 0 according as elements are all negative or not
     */
    int isneg() const { return gsl_vector_ushort_isneg( get() ); }
    /**
     * C++ version of gsl_vector_ushort_isnonneg().
     * @return @c +1 or @c 0 according as elements are all nonnegative or not
     */
    int isnonneg() const { return gsl_vector_ushort_isnonneg( get() ); }
    /**
     * C++ version of gsl_vector_ushort_get().
     * @param i index of element to get
     * @return value of element
     */
    unsigned short get( size_t const i ) const { return gsl_vector_ushort_get( get(), i ); }
    /**
     * C++ version of gsl_vector_ushort_set().
     * @param i index to set
     * @param x new value for element
     */
    void set( size_t const i, unsigned short x ){ gsl_vector_ushort_set( get(), i, x ); }
    /**
     * C++ version of gsl_vector_ushort_ptr().
     * @param i index of element to get
     * @return pointer to element
     */
    unsigned short* ptr( size_t const i ){ return gsl_vector_ushort_ptr( get(), i ); }
    /**
     * C++ version of gsl_vector_ushort_const_ptr().
     * @param i index of element to get
     * @return pointer to element
     */
    unsigned short const* const_ptr( size_t const i ) const { return gsl_vector_ushort_const_ptr( get(), i ); }
    /**
     * C++ version of gsl_vector_ushort_fread().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fread( FILE* stream ){ return gsl_vector_ushort_fread( stream, get() ); }
    /**
     * C++ version of gsl_vector_ushort_fwrite().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fwrite( FILE* stream ) const { return gsl_vector_ushort_fwrite( stream, get() ); }
    /**
     * C++ version of gsl_vector_ushort_fscanf().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fscanf( FILE* stream ){ return gsl_vector_ushort_fscanf( stream, get() ); }
    /**
    ...
    int fprintf( FILE* stream, char const* format ) const {
      return gsl_vector_ushort_fprintf( stream, get(), format ); }
    /**
     ...
     */
    vector_ushort( block_ushort& b, size_t const offset, size_t const n, size_t const stride ){
      ccgsl_pointer = gsl_vector_ushort_alloc_from_block( b.get(), offset, n, stride );
      // just plausibly we could allocate vector_ushort but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_ushort_free( ccgsl_pointer );
     ...
     * @param stride The stride
     */
    vector_ushort( vector_ushort& v, size_t const offset, size_t const n, size_t const stride ){
      ccgsl_pointer = gsl_vector_ushort_alloc_from_vector( v.get(), offset, n, stride );
      // just plausibly we could allocate vector_ushort but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_ushort_free( ccgsl_pointer );
     ...
     * @param v An array of type unsigned short
     * @param n The size of the vector_ushort
    ...
    static vector_ushort view_array( unsigned short* v, size_t n ){
      ...
      *w = gsl_vector_ushort_view_array( v, n ).vector;
    ...
    static vector_ushort view_array_with_stride( unsigned short* base, size_t stride, size_t n ){
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_view_array_with_stride( base, stride, n ).vector;
      return vector_ushort( w );
    }
    /**
     * C++ version of gsl_vector_ushort_const_view_array().
     * @param v An array of type unsigned short
     * @param n The size of the vector_ushort
     * @return A vector_ushort
     */
    static vector_ushort const const_view_array( unsigned short const* v, size_t n ){
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_view_array( v, n ).vector;
      return vector_ushort( w );
    }
    /**
    ...
    static vector_ushort const const_view_array_with_stride( unsigned short const* base, size_t stride, size_t n ){
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_view_array_with_stride( base, stride, n ).vector;
      return vector_ushort( w );
    }
    /**
    ...
    vector_ushort subvector( size_t i, size_t n ){
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_subvector( get(), i, n ).vector;
      return vector_ushort( w );
    }
    /**
    ...
    vector_ushort subvector_with_stride( size_t i, size_t stride, size_t n ){
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_ushort( w );
    }
    /**
     * C++ version of gsl_vector_ushort_subvector_with_stride().
    ...
    vector_ushort const const_subvector( size_t i, size_t n ) const {
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_subvector( get(), i, n ).vector;
      return vector_ushort( w );
    }
    /**
     * C++ version of gsl_vector_ushort_subvector_with_stride().
    ...
    vector_ushort const const_subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_ushort( w );
    }
    // Extra allocators from matrix_ushort objects. The definition must come after matrix_ushort.
    /**
     * C++ version of gsl_vector_ushort_alloc_row_from_matrix().
     * @param m A matrix_ushort
     * @param i A row
     * @return A vector_ushort
     */
   ...
   * @see vector_ushort::iterator::operator+()
   */
  inline vector_ushort::iterator operator+
  ( vector_ushort::iterator::difference_type const n, vector_ushort::iterator const& i ){ return i + n; }
   ...
   * @see vector_ushort::iterator::operator+()
   */
  inline vector_ushort::const_iterator operator+
  ( vector_ushort::const_iterator::difference_type const n, vector_ushort::const_iterator const& i ){ return i + n; }
   ...
   * @see vector_ushort::iterator::operator+()
   */
  inline vector_ushort::reverse_iterator operator+
  ( vector_ushort::reverse_iterator::difference_type const n, vector_ushort::reverse_iterator const& i ){
    return i + n; }
   ...
   * @see vector_ushort::iterator::operator+()
   */
  inline vector_ushort::const_reverse_iterator operator+
  ( vector_ushort::const_reverse_iterator::difference_type const n, vector_ushort::const_reverse_iterator const& i ){
    return i + n; }

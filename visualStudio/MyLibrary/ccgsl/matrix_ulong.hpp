...
namespace gsl {
  /**
   * This class handles matrix_ulong objects as shared handles. It models a random access container
   * so that STL functions work with matrix_ulong.
   *
   * Note that matrix_ulong_views are implemented as matrix_ulong objects here.
   */
  class matrix_ulong {
  public:
    /**
    ...
    // Refines assignable
    /**
     * The default constructor creates a new matrix_ulong with n elements
     * @param n1 The number of rows in the matrix_ulong
     * @param n2 The number of columns in the matrix_ulong
     */
    explicit matrix_ulong( size_t const n1, size_t const n2 ){
    ...
    /**
     * Could construct from a gsl_matrix_ulong. This is not usually a good idea. In this case
     * we should not use gsl_matrix_ulong_free() to deallocate the memory.
     * @param v The matrix_ulong
     */
    explicit matrix_ulong( gsl_matrix_ulong* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the matrix_ulong. Use clone() if you want a full copy.
     * @param v The matrix_ulong to copy.
     */
    matrix_ulong( matrix_ulong const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      ++*count; // matrix_ulong is now shared.
    }
    // assignment operator
    /**
    ...
    matrix_ulong& operator=( matrix_ulong const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_matrix_ulong_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
     ...
     * @return The number of rows of the matrix_ulong
     */
    size_t size1() const { return ccgsl_pointer == 0 ? 0 : ccgsl_pointer->size1; }
    /**
     * The number of columns of the matrix_ulong
     * @return The number of columns of the matrix_ulong
     */
    size_t size2() const { return ccgsl_pointer == 0 ? 0 : ccgsl_pointer->size2; }
    /**
     * Swap two matrix_ulong objects. This works even if the matrix_ulong objects have different sizes
     * because it swaps pointers.
     * @param m The matrix_ulong to swap with @c this.
     */
    void swap( matrix_ulong& m ){
      gsl_matrix_ulong* tmp = ccgsl_pointer; ccgsl_pointer = m.ccgsl_pointer; m.ccgsl_pointer = tmp;
     ...
     * @param i A row index
     * @return A row as a vector_ulong
     */
     ...
     * @return A column as a vector_ulong
     */
     ...
     * @return The principal diagonal as a vector_ulong
     */
    vector_ulong
     ...
     * @param k An index
     * @return Subdiagonal @c k as a vector_ulong
     */
     ...
     * @return A subrow as a vector_ulong
     */
     ...
     * @param n The number of elements
     * @return A subcolumn as a vector_ulong
     */
   ...
   /**
     ...
     * @return A matrix_ulong
     */
    static matrix_ulong const const_view_array( unsigned long* base, size_t const n1, size_t const n2 ){
      gsl_matrix_ulong* m = static_cast<gsl_matrix_ulong*>( malloc( sizeof( gsl_matrix_ulong ) ) );
    ...
    const_view_array_with_tda( unsigned long* base, size_t const n1, size_t const n2, size_t const tda ){
      gsl_matrix_ulong* m = static_cast<gsl_matrix_ulong*>( malloc( sizeof( gsl_matrix_ulong ) ) );
     ...
     * @return A matrix_ulong
     */
    static matrix_ulong const const_view_vector( vector_ulong& v, size_t const n1, size_t const n2 ){
      gsl_matrix_ulong* m = static_cast<gsl_matrix_ulong*>( malloc( sizeof( gsl_matrix_ulong ) ) );
     ...
     */
    static matrix_ulong const
    const_view_vector_with_tda( vector_ulong& v, size_t const n1, size_t const n2, size_t const tda ){
      gsl_matrix_ulong* m = static_cast<gsl_matrix_ulong*>( malloc( sizeof( gsl_matrix_ulong ) ) );
  ...
  public:
     ...
     * this is the only matrix_ulong object sharing the gsl_matrix_ulong
     */
    bool unique() const { return *count == 1; }
    /**
     * Find how many matrix_ulong objects share this pointer.
     * @return the number of matrix_ulong objects that share this pointer
     */
    size_t use_count() const { return *count; }
    /**
     * Allow conversion to bool.
     * @return @c true or @c false according as this contains a pointer
     * to a gsl_matrix_ulong
     */
    operator bool() const { return ccgsl_pointer != 0; }
    // GSL functions
    /**
     * C++ version of gsl_matrix_ulong_calloc(). This constructs a matrix_ulong object with entries
     * initialised to zero.
     * @param n1 The number of rows in the matrix_ulong
     * @param n2 The number of columns in the matrix_ulong
     * @return A matrix_ulong initialised to zero
     */
    ...
    /**
     * C++ version of gsl_matrix_ulong_swap_rowcol(). Swap row and column in place. Matrix must be square.
     * @param i index of row
     * @param j index of column
     * @return error code on failure
     */
     ...
     * @param src matrix_ulong whose transpose it to be copied to @c this
     * @return error code on failure
     */
     ...
     * @param imax row index of the first maximum element in the matrix_ulong
     * @param jmax column index of the first maximum element in the matrix_ulong
     */
     ...
     * @param imin row index of the first minimum element in the matrix_ulong
     * @param jmin column index of the first minimum element in the matrix_ulong
     */
    ...
    /**
     * C++ version of gsl_matrix_ulong_mul_elements(). Multiply matrices elementwise.
     * @param b Another matrix_ulong
     * @return error code on failure
     */
    int
    ...
    /**
     * C++ version of gsl_matrix_ulong_div_elements(). Divide each element of @c this by the
    ...
    // Extra functions for []
    /**
     * This function allows us to use a matrix_ulong like an array. Use with caution.
     * Although @c matrix_ulong[i][j] is possible, it is much less efficient than
     * matrix_ulong::set(). The effect is the same as row()
     * @param i The index of the row
     * @return A vector_ulong representing a row
     */
    vector_ulong operator[]( size_t const i ){
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "matrix_ulong is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return vector_ulong();
      }
      gsl_vector_ulong* w = static_cast<gsl_vector_ulong*>( malloc( sizeof( gsl_vector_ulong ) ) );
      *w = gsl_matrix_ulong_row( ccgsl_pointer, i ).vector;
      return vector_ulong( w );
    }
    /**
     * This function allows us to use a matrix_ulong like an array. Use with caution.
  ...
  vector_ulong vector_ulong::alloc_row_from_matrix( matrix_ulong& m, size_t const i ){
    return vector_ulong ( gsl_vector_ulong_alloc_row_from_matrix( m.get(), i ) ); }
   vector_ulong vector_ulong::alloc_col_from_matrix( matrix_ulong& m, size_t const i ){

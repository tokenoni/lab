...
#ifndef CCGSL_BLOCK_USHORT_HPP
#define CCGSL_BLOCK_USHORT_HPP
...
namespace gsl {
  /**
   * This class handles vector_ushorts as shared handles. It models a random access container
   * so that STL functions work with block_ushort.
   */
  class block_ushort {
  public:
    /**
     ...
     * @param n The number of elements in the block_ushort
     */
    explicit block_ushort( size_t const n ){
      ccgsl_pointer = gsl_block_ushort_alloc( n );
      // just plausibly we could allocate block_ushort but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_block_ushort_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_block_ushort. This is not usually a good idea. In this case
     * we should not use gsl_block_ushort_free() to deallocate the memory.
     * @param v The block_ushort
     */
    explicit block_ushort( gsl_block_ushort* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the block_ushort. Use clone() if you want a full copy.
     * @param v The block_ushort to copy.
     */
    block_ushort( block_ushort const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
     ...
     * @param v The block_ushort to copy
     */
    block_ushort& operator=( block_ushort const& v ){
      // first, possibly delete anything pointed to by this
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_ushort_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
    ...
    block_ushort clone() const {
      block_ushort copy( ccgsl_pointer->size );
      // Now copy
      memcpy( copy.ccgsl_pointer->data, ccgsl_pointer->data, ccgsl_pointer->size * sizeof( unsigned short ) );
      // return new object
      return copy;
    }
    // destructor
     ...
     * @param v The block_ushort to be compared with @c this
    ...
    bool operator==( block_ushort const& v ) const {
      // trivially equal if gsl_block_ushort*s are identical
     ...
     * identical elements or not
     */
    bool operator!=( block_ushort const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a block_ushort is nonnegative.
     * @param v The block_ushort to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( block_ushort const& v ) const {
      // null block_ushort comes first
	...
	unsigned short const t = ccgsl_pointer->data[i];
	unsigned short const u =v.ccgsl_pointer->data[i];
	if( t < u ) return true;
	if( u < t ) return false;
      }
      // elements match.
  ...
  private:
    ...
    public:
      // // type iterator_traits<block_ushort>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_ushort not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
  	...
  	// Warn if iterators do not point to same block_ushort
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different block_ushorts", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same block_ushort and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a block_ushort
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
       ...
       * @param v The block_ushort that creates @c this
     ...
     */
    template<bool reverse> class iterator_t : public iterator_base<block_ushort,unsigned short,reverse>{
    public:
  	...
  	iterator_base<block_ushort,unsigned short,reverse>::v = i.v;
  	iterator_base<block_ushort,unsigned short,reverse>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<block_ushort,unsigned short,reverse>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse>& operator--(){
  	iterator_base<block_ushort,unsigned short,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_ushort,unsigned short,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<block_ushort,unsigned short,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse>() : iterator_base<block_ushort,unsigned short,reverse>(){}
    protected:
      friend class block_ushort;
      // We need a constructor for block_ushort
      /**
       * This constructor allows block_ushort to create non-default iterators.
       * @param v The block_ushort that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse>( block_ushort* v, difference_type position )
      : iterator_base<block_ushort,unsigned short,reverse>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse> class const_iterator_t
      : public iterator_base<block_ushort const,unsigned short,reverse>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<block_ushort const,unsigned short,reverse>::v = i.v;
  	iterator_base<block_ushort const,unsigned short,reverse>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<block_ushort const,unsigned short,reverse>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse>& operator--(){
  	iterator_base<block_ushort const,unsigned short,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_ushort const,unsigned short,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse> const& i ) const {
  	return iterator_base<block_ushort const,unsigned short,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse>() : iterator_base<block_ushort,unsigned short,reverse>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse>( block_ushort const* v, difference_type position )
      : iterator_base<block_ushort const,unsigned short,reverse>( v, position ){}
    };
  public:
    /**
     ...
     * @return iterator pointing to first block_ushort element
     */
    iterator begin(){
     ...
     * @return iterator pointing beyond last block_ushort element
     */
    iterator end(){
     ...
     * @return The size of the block_ushort
     */
    size_type size() const { return ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the block_ushort. Identical to
    ...
    /**
     * Swap two block_ushort objects. This works even if the block_ushort objects have
     * different sizes because it swaps pointers.
     * @param v The block_ushort to swap with @c this.
     */
    void swap( block_ushort& v ){
      gsl_block_ushort* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last block_ushort element.
     * @return iterator pointing beyond last block_ushort element
     */
    ...
    unsigned short& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_ushort is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_ushort", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
   ...
   * @see block_ushort::iterator::operator+()
   */
  inline block_ushort::iterator operator+
  ( block_ushort::iterator::difference_type const n,
  			      block_ushort::iterator const& i ){
    return i + n;
  }
   ...
   * @see block_ushort::iterator::operator+()
   */
  inline block_ushort::const_iterator operator+
  ( block_ushort::const_iterator::difference_type const n,
  				    block_ushort::const_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_ushort::iterator::operator+()
   */
  inline block_ushort::reverse_iterator operator+
  ( block_ushort::reverse_iterator::difference_type const n,
  			      block_ushort::reverse_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_ushort::iterator::operator+()
   */
  inline block_ushort::const_reverse_iterator operator+
  ( block_ushort::const_reverse_iterator::difference_type const n,
  			      block_ushort::const_reverse_iterator const& i ){
    return i + n;
  }

...
#ifndef CCGSL_BLOCK_COMPLEX_FLOAT_HPP
#define CCGSL_BLOCK_COMPLEX_FLOAT_HPP
...
namespace gsl {
  /**
   * This class handles vector_complex_floats as shared handles. It models a random access container
   * so that STL functions work with block_complex_float.
   */
  ...
  public:
     ...
     * @param n The number of elements in the block_complex_float
     */
    explicit block_complex_float( size_t const n ){
      ccgsl_pointer = gsl_block_complex_float_calloc( n );
      // just plausibly we could allocate block_complex_float but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_block_complex_float_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_block_complex_float. This is not usually a good idea. In this case
     * we should not use gsl_block_complex_float_free() to deallocate the memory.
     * @param v The block_complex_float
     */
    explicit block_complex_float( gsl_block_complex_float* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the block_complex_float. Use clone() if you want a full copy.
     * @param v The block_complex_float to copy.
     */
    block_complex_float( block_complex_float const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
    ...
    block_complex_float& operator=( block_complex_float const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_complex_float_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
    ...
    block_complex_float clone() const {
      block_complex_float copy( ccgsl_pointer->size );
      // Now copy
      memcpy( copy.ccgsl_pointer->data, ccgsl_pointer->data,
	      CCGSL_MTY * ccgsl_pointer->size * sizeof( float ) );
      // return new object
      return copy;
    }
    // destructor
     ...
     * @param v The block_complex_float to be compared with @c this
    ...
    bool operator==( block_complex_float const& v ) const {
      // trivially equal if gsl_block_complex_float*s are identical
     ...
     * identical elements or not
     */
    bool operator!=( block_complex_float const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a block_complex_float is nonnegative.
     * @param v The block_complex_float to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( block_complex_float const& v ) const {
      // null block_complex_float comes first
  ...
  private:
    ...
    public:
	  ...
	  gsl_error( "block_complex_float not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return complex_float_ref( 0 );
  	}
	...
	return complex_float_ref( v->ccgsl_pointer->data + CCGSL_MTY * position );
      }
      // // operator->
      /**
       * Dereference the pointer.
  	...
  	return complex_float_ref( v->ccgsl_pointer->data + CCGSL_MTY * p );
      }
  	...
  	// Warn if iterators do not point to same block_complex_float
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different block_complex_floats", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same block_complex_float and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a block_complex_float
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
       ...
       * @param v The block_complex_float that creates @c this
     ...
     */
    template<bool reverse> class iterator_t : public iterator_base<block_complex_float,float,reverse>{
    public:
  	...
  	iterator_base<block_complex_float,float,reverse>::v = i.v;
  	iterator_base<block_complex_float,float,reverse>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<block_complex_float,float,reverse>::increment();
  	return *this;
      }
      /**
  	...
  	iterator_base<block_complex_float,complex_float,reverse>::increment();
  	return result;
      }
       ...
       */
      iterator_t<reverse>& operator--(){
  	iterator_base<block_complex_float,float,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_complex_float,float,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<block_complex_float,float,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse>() : iterator_base<block_complex_float,complex_float,reverse>(){}
    protected:
      friend class block_complex_float;
      // We need a constructor for block_complex_float
      /**
       * This constructor allows block_complex_float to create non-default iterators.
       * @param v The block_complex_float that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse>( block_complex_float* v, difference_type position )
      : iterator_base<block_complex_float,float,reverse>( v, position ){}
    };
    /**
    ...
    public:
  	...
  	iterator_base<block_complex_float const,float,reverse>::v = i.v;
  	iterator_base<block_complex_float const,float,reverse>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<block_complex_float const,float,reverse>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse>& operator--(){
  	iterator_base<block_complex_float const,float,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_complex_float const,float,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse> const& i ) const {
  	return iterator_base<block_complex_float const,float,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse>() : iterator_base<block_complex_float,float,reverse>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse>( block_complex_float const* v, difference_type position )
      : iterator_base<block_complex_float const,float,reverse>( v, position ){}
    };
  public:
    /**
     ...
     * @return iterator pointing to first block_complex_float element
     */
    iterator begin(){
     ...
     * @return iterator pointing beyond last block_complex_float element
     */
    iterator end(){
     ...
     * @return The size of the block_complex_float
     */
    size_type size() const { return ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the block_complex_float. Identical to
    ...
    /**
     * Swap two block_complex_float objects. This works even if the block_complex_float objects have
     * different sizes because it swaps pointers.
     * @param v The block_complex_float to swap with @c this.
     */
    void swap( block_complex_float& v ){
      gsl_block_complex_float* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last block_complex_float element.
     * @return iterator pointing beyond last block_complex_float element
     */
    ...
    reference operator[]( size_t const n ){
      ...
      static complex_float something;
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_complex_float is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_complex_float", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // n is a valid position
      return complex_float_ref( ccgsl_pointer->data + CCGSL_MTY * n );
    }
    /**
     * Get element at position @c n by reference ([] operator).
     * @param n The position of the element
   ...
   * @see block_complex_float::iterator::operator+()
   */
  inline block_complex_float::iterator operator+
  ( block_complex_float::iterator::difference_type const n,
  			      block_complex_float::iterator const& i ){
    return i + n;
  }
   ...
   * @see block_complex_float::iterator::operator+()
   */
  inline block_complex_float::const_iterator operator+
  ( block_complex_float::const_iterator::difference_type const n,
  				    block_complex_float::const_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_complex_float::iterator::operator+()
   */
  inline block_complex_float::reverse_iterator operator+
  ( block_complex_float::reverse_iterator::difference_type const n,
  			      block_complex_float::reverse_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_complex_float::iterator::operator+()
   */
  inline block_complex_float::const_reverse_iterator operator+
  ( block_complex_float::const_reverse_iterator::difference_type const n,
  			      block_complex_float::const_reverse_iterator const& i ){
    return i + n;
  }

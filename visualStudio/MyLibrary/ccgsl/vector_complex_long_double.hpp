...
namespace gsl {
  ...
  public:
    ...
    explicit vector_complex_long_double( size_t const n ){
      ccgsl_pointer = gsl_vector_complex_long_double_calloc( n );
     ...
     * we should not use gsl_vector_complex_long_double_free() to deallocate the memory.
    ...
    vector_complex_long_double( vector_complex_long_double const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      ++*count; // vector_complex_long_double is now shared.
    }
    // assignment operator
    /**
    ...
    vector_complex_long_double& operator=( vector_complex_long_double const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_vector_complex_long_double_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
  ...
  public:
     ...
     */
    vector_complex_long_double( block_complex_long_double& b, size_t const offset, size_t const n, size_t const stride ){
     ...
     * @param stride The stride
     */
    vector_complex_long_double( vector_complex_long_double& v, size_t const offset, size_t const n, size_t const stride ){

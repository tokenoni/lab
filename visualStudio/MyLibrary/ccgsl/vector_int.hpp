...
namespace gsl {
  ...
  class matrix_int;
  /**
   * This class handles vector_int objects as shared handles. It models a random access container
   * so that STL functions work with vector_int.
   *
  ...
  public:
     ...
     * @param n The number of elements in the vector_int
     */
    explicit vector_int( size_t const n ){
    ...
    /**
     * Could construct from a gsl_vector_int. This is not usually a good idea. In this case
     * we should not use gsl_vector_int_free() to deallocate the memory.
     * @param v The vector_int
     */
    explicit vector_int( gsl_vector_int* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the vector_int. Use clone() if you want a full copy.
     * @param v The vector_int to copy.
     */
    vector_int( vector_int const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      ++*count; // vector_int is now shared.
    }
    // assignment operator
    /**
    ...
    vector_int& operator=( vector_int const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_vector_int_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
  ...
  private:
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same vector_int and the function calls
    ...
    public:
       ...
       */
      iterator_t<reverse_t>() : iterator_base<vector_int,int,reverse_t>(){}
    protected:
      friend class vector_int;
       ...
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse_t>( vector_int* v, difference_type position )
    ...
    public:
       ...
       * The default constructor.
       */
      const_iterator_t<reverse_t>() : iterator_base<vector_int,int,reverse_t>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse_t>( vector_int const* v, difference_type position )
  ...
  public:
     ...
     * @return The size of the vector_int
     */
    size_type size() const { return ccgsl_pointer == 0 ? 0 : ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the vector_int. Identical to
    ...
    /**
     * Swap two vector_int objects. This works even if the vector_int objects have different sizes
     * because it swaps pointers.
     * @param v The vector_int to swap with @c this.
     */
    void swap( vector_int& v ){
      gsl_vector_int* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last vector_int element.
     * @return iterator pointing beyond last vector_int element
     */
    ...
    int& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_int is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_int", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
  ...
  public:
     ...
     * @return the number of vector_int objects that share this pointer
     */
    ...
    // GSL functions
    /**
     * C++ version of gsl_vector_int_calloc(). This constructs a vector_int object with entries
     * initialised to zero.
     * @param n The size of the vector_int
     * @return A vector_int initialised to zero
     */
    ...
    /**
     * C++ version of gsl_vector_int_set_basis(). Creates a basis vector_int with one nonzero element.
     ...
     */
    vector_int( block_int& b, size_t const offset, size_t const n, size_t const stride ){
     ...
     * @param stride The stride
     */
    vector_int( vector_int& v, size_t const offset, size_t const n, size_t const stride ){
    ...
    /**
     * C++ version of gsl_vector_int_const_view_array().
     * @param v An array of type int
     * @param n The size of the vector_int
     * @return A vector_int
     */
   ...
   * @see vector_int::iterator::operator+()
   */

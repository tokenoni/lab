...
#ifndef CCGSL_BLOCK_CHAR_HPP
#define CCGSL_BLOCK_CHAR_HPP
...
namespace gsl {
  /**
   * This class handles vector_chars as shared handles. It models a random access container
   * so that STL functions work with block_char.
   */
  class block_char {
  public:
    /**
    ...
    // Refines assignable
    /**
     * The default constructor creates a new block_char with n elements
     * @param n The number of elements in the block_char
     */
    explicit block_char( size_t const n ){
      ccgsl_pointer = gsl_block_char_alloc( n );
      // just plausibly we could allocate block_char but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_block_char_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_block_char. This is not usually a good idea. In this case
     * we should not use gsl_block_char_free() to deallocate the memory.
     * @param v The block_char
     */
    explicit block_char( gsl_block_char* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the block_char. Use clone() if you want a full copy.
     * @param v The block_char to copy.
     */
    block_char( block_char const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
     ...
     * @param v The block_char to copy
     */
    block_char& operator=( block_char const& v ){
      // first, possibly delete anything pointed to by this
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_char_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
     ...
     */
    block_char clone() const {
      block_char copy( ccgsl_pointer->size );
      // Now copy
      memcpy( copy.ccgsl_pointer->data, ccgsl_pointer->data, ccgsl_pointer->size * sizeof( char ) );
      // return new object
      return copy;
    }
    // destructor
    ...
    // == operator
    /**
     * Two block_char objects are identically equal if their elements are identical.
     * @param v The block_char to be compared with @c this
     * @return @c true or @c false according as @c this and @c v have
     * identical elements or not
     */
    bool operator==( block_char const& v ) const {
      // trivially equal if gsl_block_char*s are identical
    ...
    // != operator
    /**
     * Two block_char objects are different equal if their elements are not identical.
     * @param v The block_char to be compared with @c this
     * @return @c false or @c true according as @c this and @c v have
     * identical elements or not
     */
    bool operator!=( block_char const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a block_char is nonnegative.
     * @param v The block_char to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( block_char const& v ) const {
      // null block_char comes first
	...
	char const t = ccgsl_pointer->data[i];
	char const u =v.ccgsl_pointer->data[i];
	if( t < u ) return true;
	if( u < t ) return false;
      }
      // elements match.
  ...
  private:
    ...
    class iterator_base
      ...
      friend class block_char;
    public:
      // // type iterator_traits<block_char>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_char not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
  	...
  	// Warn if iterators do not point to same block_char
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different block_chars", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same block_char and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a block_char
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
      ...
      /**
       * This constructor allows block_char to create non-default iterators.
       * @param v The block_char that creates @c this
      ...
      /**
       * Store a pointer to a block_char we can iterate over: 0 if no block_char.
       */
      container* v;
      /**
       * Mark position of iterator within block_char.
       */
     ...
     */
    template<bool reverse> class iterator_t : public iterator_base<block_char,char,reverse>{
    public:
  	...
  	iterator_base<block_char,char,reverse>::v = i.v;
  	iterator_base<block_char,char,reverse>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<block_char,char,reverse>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse>& operator--(){
  	iterator_base<block_char,char,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_char,char,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<block_char,char,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse>() : iterator_base<block_char,char,reverse>(){}
    protected:
      friend class block_char;
      // We need a constructor for block_char
      /**
       * This constructor allows block_char to create non-default iterators.
       * @param v The block_char that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse>( block_char* v, difference_type position )
      : iterator_base<block_char,char,reverse>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse> class const_iterator_t
      : public iterator_base<block_char const,char,reverse>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<block_char const,char,reverse>::v = i.v;
  	iterator_base<block_char const,char,reverse>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<block_char const,char,reverse>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse>& operator--(){
  	iterator_base<block_char const,char,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_char const,char,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse> const& i ) const {
  	return iterator_base<block_char const,char,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse>() : iterator_base<block_char,char,reverse>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse>( block_char const* v, difference_type position )
      : iterator_base<block_char const,char,reverse>( v, position ){}
    };
  public:
    /**
    ...
    // begin()
    /**
     * Get iterator pointing to first block_char element.
     * @return iterator pointing to first block_char element
     */
    iterator begin(){
    ...
    // end()
    /**
     * Get iterator pointing beyond last block_char element.
     * @return iterator pointing beyond last block_char element
     */
    iterator end(){
    ...
    // size()
    /**
     * The size (number of elements) of the block_char.
     * @return The size of the block_char
     */
    size_type size() const { return ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the block_char. Identical to
    ...
    // empty()
    /**
     * Find if the block_char is empty.
    ...
    /**
     * Swap two block_char objects. This works even if the block_char objects have
     * different sizes because it swaps pointers.
     * @param v The block_char to swap with @c this.
     */
    void swap( block_char& v ){
      gsl_block_char* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last block_char element.
     * @return iterator pointing beyond last block_char element
     */
    ...
    char& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_char is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_char", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
  ...
  public:
    ...
    /**
     * Get the gsl_block_char.
     * @return the gsl_block_char
     */
    gsl_block_char* get() const { return ccgsl_pointer; }
    /**
     * Find if this is the only object sharing the gsl_block_char.
     * @return @c true or @c falses according as 
     * this is the only block_char object sharing the gsl_block_char
     */
    bool unique() const { return *count == 1; }
    /**
     * Find how many block_char objects share this pointer.
     * @return the number of block_char objects that share this pointer
     */
   ...
   * @see block_char::iterator::operator+()
   */
  inline block_char::iterator operator+
  ( block_char::iterator::difference_type const n,
   ...
   * @see block_char::iterator::operator+()
   */
  inline block_char::const_iterator operator+
  ( block_char::const_iterator::difference_type const n,
  				    block_char::const_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_char::iterator::operator+()
   */
  inline block_char::reverse_iterator operator+
  ( block_char::reverse_iterator::difference_type const n,
  			      block_char::reverse_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_char::iterator::operator+()
   */
  inline block_char::const_reverse_iterator operator+
  ( block_char::const_reverse_iterator::difference_type const n,
  			      block_char::const_reverse_iterator const& i ){
    return i + n;
  }

...
#ifndef CCGSL_VECTOR_COMPLEX_FLOAT_HPP
#define CCGSL_VECTOR_COMPLEX_FLOAT_HPP
...
#include"block_complex_float.hpp"

// This file is a template
#define CCGSL_MTY 2

namespace gsl {
  // declare matrix_complex_float class
  class matrix_complex_float;
  /**
   * This class handles vector_complex_float objects as shared handles. It models a random access container
   * so that STL functions work with vector_complex_float.
   *
  ...
  public:
     ...
     * @param n The number of elements in the vector_complex_float
     */
    explicit vector_complex_float( size_t const n ){
      ccgsl_pointer = gsl_vector_complex_float_calloc( n );
      // just plausibly we could allocate vector_complex_float but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_complex_float_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_vector_complex_float. This is not usually a good idea. In this case
     * we should not use gsl_vector_complex_float_free() to deallocate the memory.
     * @param v The vector_complex_float
     */
    explicit vector_complex_float( gsl_vector_complex_float* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the vector_complex_float. Use clone() if you want a full copy.
     * @param v The vector_complex_float to copy.
     */
    vector_complex_float( vector_complex_float const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      ++*count; // vector_complex_float is now shared.
    }
    // assignment operator
    /**
    ...
    vector_complex_float& operator=( vector_complex_float const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_vector_complex_float_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
    ...
    vector_complex_float clone() const {
      vector_complex_float copy( get()->size );
      // Now copy
      gsl_vector_complex_float_memcpy( copy.get(), get() );
      // return new object
      return copy;
    }
    // destructor
     ...
     * @param v The vector_complex_float to be compared with @c this
    ...
    bool operator==( vector_complex_float const& v ) const {
      // trivially equal if gsl_vector_complex_float*s are identical
     ...
     * identical elements or not
     */
    bool operator!=( vector_complex_float const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a vector_complex_float is nonnegative.
     * @param v The vector_complex_float to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( vector_complex_float const& v ) const {
      // null vector_complex_float comes first
	...
	complex_float const t = gsl_vector_complex_float_get( ccgsl_pointer, i );
	complex_float const u =gsl_vector_complex_float_get( v.ccgsl_pointer, i );
	if( t < u ) return true;
	if( u < t ) return false;
      }
      // elements match.
  ...
  private:
    ...
    public:
      ...
      // // type iterator_traits<block_complex_float>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  return complex_float_ref( 0 );
  	} else if( v->ccgsl_pointer == 0 ){
  	  gsl_error( "vector_complex_float not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return complex_float_ref( 0 );
  	}
  	...
  	return complex_float_ref( v->ccgsl_pointer->data + CCGSL_MTY * position * v->ccgsl_pointer->stride );
      }
      // // operator->
      /**
       * Dereference the pointer.
  	...
  	// Warn if iterators do not point to same vector_complex_float
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different vector_complex_float objects", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same vector_complex_float and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a vector_complex_float
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
     ...
     */
    template<bool reverse_t> class iterator_t : public iterator_base<vector_complex_float,float,reverse_t>{
    public:
  	...
  	iterator_base<vector_complex_float,float,reverse_t>::v = i.v;
  	iterator_base<vector_complex_float,float,reverse_t>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<vector_complex_float,float,reverse_t>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse_t>& operator--(){
  	iterator_base<vector_complex_float,float,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<vector_complex_float,float,reverse_t>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<vector_complex_float,float,reverse_t>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse_t>() : iterator_base<vector_complex_float,float,reverse_t>(){}
    protected:
      friend class vector_complex_float;
      // We need a constructor for vector_complex_float
      /**
       * This constructor allows vector_complex_float to create non-default iterators.
       * @param v The vector_complex_float that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse_t>( vector_complex_float* v, difference_type position )
      : iterator_base<vector_complex_float,float,reverse_t>( v, position ){}
    };
    /**
    ...
    public:
  	...
  	iterator_base<vector_complex_float const,float,reverse_t>::v = i.v;
  	iterator_base<vector_complex_float const,float,reverse_t>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<vector_complex_float const,float,reverse_t>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse_t>& operator--(){
  	iterator_base<vector_complex_float const,float,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<vector_complex_float const,float,reverse_t>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse_t> const& i ) const {
  	return iterator_base<vector_complex_float const,float,reverse_t>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse_t>() : iterator_base<vector_complex_float,float,reverse_t>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse_t>( vector_complex_float const* v, difference_type position )
      : iterator_base<vector_complex_float const,float,reverse_t>( v, position ){}
    };
  public:
    /**
     ...
     * @return The size of the vector_complex_float
     */
    size_type size() const { return ccgsl_pointer == 0 ? 0 : ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the vector_complex_float. Identical to
    ...
    /**
     * Swap two vector_complex_float objects. This works even if the vector_complex_float objects have different sizes
     * because it swaps pointers.
     * @param v The vector_complex_float to swap with @c this.
     */
    void swap( vector_complex_float& v ){
      gsl_vector_complex_float* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last vector_complex_float element.
     * @return iterator pointing beyond last vector_complex_float element
     */
     ...
     * @return a reference to a complex_float
     */
    complex_float_ref operator[]( size_t const n ){
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_complex_float is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return complex_float_ref( 0 );
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_complex_float", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return complex_float_ref( 0 );
      }
      // n is a valid position
      return complex_float_ref( ccgsl_pointer->data + CCGSL_MTY * n * ccgsl_pointer->stride );
    }
    /**
     * Get element at position @c n by reference ([] operator).
     * @param n The position of the element
    ...
    complex_float_ref const  operator[]( size_t const n ) const {
      ...
      // n is a valid position
      return complex_float_ref( ccgsl_pointer->data + n * ccgsl_pointer->stride );
    }
  private:
    /**
     * The shared pointer
  ...
  public:
     ...
     * @return the gsl_vector_complex_float
     */
    ...
    gsl_vector_complex_float const* get() const {
      ...
      return ccgsl_pointer; }
    /**
     * Find if @c this is the only object sharing the gsl_vector_complex_float.
     * @return @c true or @c falses according as 
     * this is the only vector_complex_float object sharing the gsl_vector_complex_float
     */
    bool unique() const { return *count == 1; }
    /**
     * Find how many vector_complex_float objects share this pointer.
     * @return the number of vector_complex_float objects that share this pointer
     */
    size_t use_count() const { return *count; }
    /**
     * Allow conversion to bool.
     * @return @c true or @c false according as @c this contains a pointer
     * to a gsl_vector_complex_float
     */
    operator bool() const { return ccgsl_pointer != 0; }

    // GSL functions
    /**
     * C++ version of gsl_vector_complex_float_calloc(). This constructs a vector_complex_float object with entries
     * initialised to zero.
     * @param n The size of the vector_complex_float
     * @return A vector_complex_float initialised to zero
     */
    static vector_complex_float calloc( size_t const n ){ return vector_complex_float( gsl_vector_complex_float_calloc( n ) ); }
    /**
     * C++ version of gsl_vector_complex_float_set_zero().
     */
    void set_zero(){ gsl_vector_complex_float_set_zero( get() ); }
    /**
     * C++ version of gsl_vector_complex_float_set_all().
     * @param x The value to which all elements are set
     */
    void set_all( complex_float x ){ gsl_vector_complex_float_set_all( get(), x ); }
    /**
     * C++ version of gsl_vector_complex_float_set_basis(). Creates a basis vector_complex_float with one nonzero element.
     * @param i The element to be set to 1.
     * @return error code on failure
     */
    int set_basis( size_t i ){ return gsl_vector_complex_float_set_basis( get(), i ); }
    /**
     * C++ version of gsl_vector_complex_float_memcpy().
     * @param src source vector_complex_float
     * @return error code on failure
     */
    int memcpy( vector_complex_float const& src ){ return gsl_vector_complex_float_memcpy( get(), src.get() ); }
    /**
     * C++ version of gsl_vector_complex_float_reverse().
     * @return error code on failure
     */
    int reverse(){ return gsl_vector_complex_float_reverse( get() ); }
    /**
    ...
    int swap_elements( size_t const i, size_t const j ){
      return gsl_vector_complex_float_swap_elements( get(), i, j ); }
    /**
     * C++ version of gsl_vector_complex_float_add().
     * @param b vector_complex_float to add to @c this
     * @return error code on failure
     */
    int add( vector_complex_float const& b ){ return gsl_vector_complex_float_add( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_complex_float_sub().
     * @param b vector_complex_float to subtract from @c this
     * @return error code on failure
     */
    int sub( vector_complex_float const& b ){ return gsl_vector_complex_float_sub( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_complex_float_mul().
     * @param b vector_complex_float to multiply elementwise with @c this
     * @return error code on failure
     */
    int mul( vector_complex_float const& b ){ return gsl_vector_complex_float_mul( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_complex_float_div().
     * @param b vector_complex_float to dividev @c this by elementwise
     * @return error code on failure
     */
    int div( vector_complex_float const& b ){ return gsl_vector_complex_float_div( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_complex_float_scale().
     * @param x constant to multiply @c this by
     * @return error code on failure
     */
    int scale( complex_float const x ){ return gsl_vector_complex_float_scale( get(), x ); }
    /**
     * C++ version of gsl_vector_complex_float_add_constant().
     * @param x constant to add to each element of @c this
     * @return error code on failure
     */
    int add_constant( complex_float const x ){ return gsl_vector_complex_float_add_constant( get(), x ); }
    /**
     * C++ version of gsl_vector_complex_float_isnull().
     * @return @c +1 or @c 0 according as elements are all zero or not
     */
    int isnull() const { return gsl_vector_complex_float_isnull( get() ); }
    /**
     * C++ version of gsl_vector_complex_float_ispos().
     * @return @c +1 or @c 0 according as elements are all positive or not
     */
    int ispos() const { return gsl_vector_complex_float_ispos( get() ); }
    /**
     * C++ version of gsl_vector_complex_float_isneg().
     * @return @c +1 or @c 0 according as elements are all negative or not
     */
    int isneg() const { return gsl_vector_complex_float_isneg( get() ); }
    /**
     * C++ version of gsl_vector_complex_float_isnonneg().
     * @return @c +1 or @c 0 according as elements are all nonnegative or not
     */
    int isnonneg() const { return gsl_vector_complex_float_isnonneg( get() ); }
    /**
     * C++ version of gsl_vector_complex_float_get().
     * @param i index of element to get
     * @return value of element
     */
    complex_float get( size_t const i ) const { return gsl_vector_complex_float_get( get(), i ); }
    /**
     * C++ version of gsl_vector_complex_float_set().
     * @param i index to set
     * @param x new value for element
     */
    void set( size_t const i, complex_float x ){ gsl_vector_complex_float_set( get(), i, x ); }
    /**
	...
	gsl_error( "Index out of range", __FILE__, __LINE__, exception::GSL_EINVAL );
      return complex_float_ptr( ccgsl_pointer->data + CCGSL_MTY * i ); }
    /**
     ...
     * @return error code on failure
     */
    int fread( FILE* stream ){ return gsl_vector_complex_float_fread( stream, get() ); }
    /**
     * C++ version of gsl_vector_complex_float_fwrite().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fwrite( FILE* stream ) const { return gsl_vector_complex_float_fwrite( stream, get() ); }
    /**
     * C++ version of gsl_vector_complex_float_fscanf().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fscanf( FILE* stream ){ return gsl_vector_complex_float_fscanf( stream, get() ); }
    /**
    ...
    int fprintf( FILE* stream, char const* format ) const {
      return gsl_vector_complex_float_fprintf( stream, get(), format ); }
    /**
     ...
     */
    vector_complex_float( block_complex_float& b, size_t const offset, size_t const n, size_t const stride ){
      ccgsl_pointer = gsl_vector_complex_float_alloc_from_block( b.get(), offset, n, stride );
      // just plausibly we could allocate vector_complex_float but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_complex_float_free( ccgsl_pointer );
     ...
     * @param stride The stride
     */
    vector_complex_float( vector_complex_float& v, size_t const offset, size_t const n, size_t const stride ){
      ccgsl_pointer = gsl_vector_complex_float_alloc_from_vector( v.get(), offset, n, stride );
      // just plausibly we could allocate vector_complex_float but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_complex_float_free( ccgsl_pointer );
    ...
    static vector_complex_float view_array( float* v, size_t n ){
      ...
      *w = gsl_vector_complex_float_view_array( v, n ).vector;
    ...
    static vector_complex_float view_array_with_stride( float* base, size_t stride, size_t n ){
      gsl_vector_complex_float* w = static_cast<gsl_vector_complex_float*>( malloc( sizeof( gsl_vector_complex_float ) ) );
      *w = gsl_vector_complex_float_view_array_with_stride( base, stride, n ).vector;
      return vector_complex_float( w );
    }
    /**
     * C++ version of gsl_vector_complex_float_const_view_array().
     * @param v An array of type float
     * @param n The size of the vector_complex_float
     * @return A vector_complex_float
     */
    static vector_complex_float const const_view_array( float const* v, size_t n ){
      gsl_vector_complex_float* w = static_cast<gsl_vector_complex_float*>( malloc( sizeof( gsl_vector_complex_float ) ) );
      *w = gsl_vector_complex_float_const_view_array( v, n ).vector;
      return vector_complex_float( w );
    }
    /**
    ...
    static vector_complex_float const const_view_array_with_stride( float const* base, size_t stride, size_t n ){
      gsl_vector_complex_float* w = static_cast<gsl_vector_complex_float*>( malloc( sizeof( gsl_vector_complex_float ) ) );
      *w = gsl_vector_complex_float_const_view_array_with_stride( base, stride, n ).vector;
      return vector_complex_float( w );
    }
    /**
    ...
    vector_complex_float subvector( size_t i, size_t n ){
      gsl_vector_complex_float* w = static_cast<gsl_vector_complex_float*>( malloc( sizeof( gsl_vector_complex_float ) ) );
      *w = gsl_vector_complex_float_subvector( get(), i, n ).vector;
      return vector_complex_float( w );
    }
    /**
    ...
    vector_complex_float subvector_with_stride( size_t i, size_t stride, size_t n ){
      gsl_vector_complex_float* w = static_cast<gsl_vector_complex_float*>( malloc( sizeof( gsl_vector_complex_float ) ) );
      *w = gsl_vector_complex_float_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_complex_float( w );
    }
    /**
     * C++ version of gsl_vector_complex_float_subvector_with_stride().
    ...
    vector_complex_float const const_subvector( size_t i, size_t n ) const {
      gsl_vector_complex_float* w = static_cast<gsl_vector_complex_float*>( malloc( sizeof( gsl_vector_complex_float ) ) );
      *w = gsl_vector_complex_float_const_subvector( get(), i, n ).vector;
      return vector_complex_float( w );
    }
    /**
     * C++ version of gsl_vector_complex_float_subvector_with_stride().
    ...
    vector_complex_float const const_subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_complex_float* w = static_cast<gsl_vector_complex_float*>( malloc( sizeof( gsl_vector_complex_float ) ) );
      *w = gsl_vector_complex_float_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_complex_float( w );
    }
    // Extra allocators from matrix_complex_float objects. The definition must come after matrix_complex_float.
    /**
     * C++ version of gsl_vector_complex_float_alloc_row_from_matrix().
     * @param m A matrix_complex_float
     * @param i A row
     * @return A vector_complex_float
     */
   ...
   * @see vector_complex_float::iterator::operator+()
   */
  inline vector_complex_float::iterator operator+
  ( vector_complex_float::iterator::difference_type const n, vector_complex_float::iterator const& i ){ return i + n; }
   ...
   * @see vector_complex_float::iterator::operator+()
   */
  inline vector_complex_float::const_iterator operator+
  ( vector_complex_float::const_iterator::difference_type const n, vector_complex_float::const_iterator const& i ){ return i + n; }
   ...
   * @see vector_complex_float::iterator::operator+()
   */
  inline vector_complex_float::reverse_iterator operator+
  ( vector_complex_float::reverse_iterator::difference_type const n, vector_complex_float::reverse_iterator const& i ){
    return i + n; }
   ...
   * @see vector_complex_float::iterator::operator+()
   */
  inline vector_complex_float::const_reverse_iterator operator+
  ( vector_complex_float::const_reverse_iterator::difference_type const n, vector_complex_float::const_reverse_iterator const& i ){
    return i + n; }

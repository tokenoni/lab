...
#ifndef CCGSL_VECTOR_LONG_HPP
#define CCGSL_VECTOR_LONG_HPP
...
namespace gsl {
  // declare matrix_long class
  class matrix_long;
  /**
   * This class handles vector_long objects as shared handles. It models a random access container
   * so that STL functions work with vector_long.
   *
  ...
  public:
	...
	gsl_vector_long_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    ...
    vector_long( vector_long const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      ++*count; // vector_long is now shared.
    }
    // assignment operator
    /**
    ...
    vector_long& operator=( vector_long const& v ){
      ...
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_vector_long_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
    ...
    vector_long clone() const {
      vector_long copy( get()->size );
      // Now copy
      gsl_vector_long_memcpy( copy.get(), get() );
      // return new object
      return copy;
    }
    // destructor
    ...
    bool operator==( vector_long const& v ) const {
      // trivially equal if gsl_vector_long*s are identical
     ...
     * identical elements or not
     */
    bool operator!=( vector_long const& v ) const { return not operator==( v ); }
	...
	long const t = gsl_vector_long_get( ccgsl_pointer, i );
	long const u =gsl_vector_long_get( v.ccgsl_pointer, i );
	if( t < u ) return true;
	if( u < t ) return false;
      }
      // elements match.
  ...
  private:
    ...
    public:
      // // type iterator_traits<vector_long>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "vector_long not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
  	...
  	// Warn if iterators do not point to same vector_long
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different vector_long objects", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same vector_long and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a vector_long
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
      ...
      /**
       * Store a pointer to a vector_long we can iterate over: 0 if no vector_long.
       */
      container* v;
      /**
       * Mark position of iterator within vector_long.
       */
     ...
     */
    template<bool reverse_t> class iterator_t : public iterator_base<vector_long,long,reverse_t>{
    public:
  	...
  	iterator_base<vector_long,long,reverse_t>::v = i.v;
  	iterator_base<vector_long,long,reverse_t>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<vector_long,long,reverse_t>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse_t>& operator--(){
  	iterator_base<vector_long,long,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<vector_long,long,reverse_t>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<vector_long,long,reverse_t>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse_t>() : iterator_base<vector_long,long,reverse_t>(){}
    protected:
      friend class vector_long;
      // We need a constructor for vector_long
      /**
       * This constructor allows vector_long to create non-default iterators.
       * @param v The vector_long that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse_t>( vector_long* v, difference_type position )
      : iterator_base<vector_long,long,reverse_t>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse_t> class const_iterator_t
      : public iterator_base<vector_long const,long,reverse_t>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<vector_long const,long,reverse_t>::v = i.v;
  	iterator_base<vector_long const,long,reverse_t>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<vector_long const,long,reverse_t>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse_t>& operator--(){
  	iterator_base<vector_long const,long,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<vector_long const,long,reverse_t>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse_t> const& i ) const {
  	return iterator_base<vector_long const,long,reverse_t>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse_t>() : iterator_base<vector_long,long,reverse_t>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse_t>( vector_long const* v, difference_type position )
      : iterator_base<vector_long const,long,reverse_t>( v, position ){}
    };
  public:
    /**
     ...
     * @return The size of the vector_long
     */
    size_type size() const { return ccgsl_pointer == 0 ? 0 : ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the vector_long. Identical to
    ...
    /**
     * Swap two vector_long objects. This works even if the vector_long objects have different sizes
     * because it swaps pointers.
     * @param v The vector_long to swap with @c this.
     */
    void swap( vector_long& v ){
      gsl_vector_long* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last vector_long element.
     * @return iterator pointing beyond last vector_long element
     */
     ...
     * @return a reference to a long
     */
    long& operator[]( size_t const n ){
      // Always try to return something
      static long something = 0;
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_long is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_long", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
  ...
  public:
     ...
     * @return the number of vector_long objects that share this pointer
     */
    size_t use_count() const { return *count; }
    /**
     * Allow conversion to bool.
     * @return @c true or @c false according as @c this contains a pointer
     * to a gsl_vector_long
     */
    operator bool() const { return ccgsl_pointer != 0; }

    // GSL functions
    /**
     * C++ version of gsl_vector_long_calloc(). This constructs a vector_long object with entries
     * initialised to zero.
     * @param n The size of the vector_long
     * @return A vector_long initialised to zero
     */
    static vector_long calloc( size_t const n ){ return vector_long( gsl_vector_long_calloc( n ) ); }
    /**
     * C++ version of gsl_vector_long_set_zero().
     */
    void set_zero(){ gsl_vector_long_set_zero( get() ); }
    /**
     * C++ version of gsl_vector_long_set_all().
     * @param x The value to which all elements are set
     */
    void set_all( long x ){ gsl_vector_long_set_all( get(), x ); }
    /**
     * C++ version of gsl_vector_long_set_basis(). Creates a basis vector_long with one nonzero element.
     * @param i The element to be set to 1.
     * @return error code on failure
     */
    int set_basis( size_t i ){ return gsl_vector_long_set_basis( get(), i ); }
    /**
     * C++ version of gsl_vector_long_memcpy().
     * @param src source vector_long
     * @return error code on failure
     */
    int memcpy( vector_long const& src ){ return gsl_vector_long_memcpy( get(), src.get() ); }
    /**
     * C++ version of gsl_vector_long_reverse().
     * @return error code on failure
     */
    int reverse(){ return gsl_vector_long_reverse( get() ); }
    /**
    ...
    int swap_elements( size_t const i, size_t const j ){
      return gsl_vector_long_swap_elements( get(), i, j ); }
    /**
     * C++ version of gsl_vector_long_max().
     * @return maximum element of vector_long
     */
    long max() const { return gsl_vector_long_max( get() ); }
    /**
     * C++ version of gsl_vector_long_min().
     * @return minimum element of vector_long
     */
    long min() const { return gsl_vector_long_min( get() ); }
    /**
     * C++ version of gsl_vector_long_minmax().
     * @param min_out minimum element of vector_long
     * @param max_out maximum element of vector_long
     */
    void minmax( long* min_out, long* max_out ) const {
      gsl_vector_long_minmax( get(), min_out, max_out ); }
    /**
     * C++ version of gsl_vector_long_max_index().
     * @return index of maximum value of vector_long
     */
    size_t max_index() const { return gsl_vector_long_max_index( get() ); }
    /**
     * C++ version of gsl_vector_long_min_index().
     * @return index of minimum value of vector_long
     */
    size_t min_index() const { return gsl_vector_long_min_index( get() ); }
    /**
     * C++ version of gsl_vector_long_minmax_index().
     * @param imin index of minimum value of vector_long
     * @param imax index of maximum value of vector_long
     */
    void minmax_index( size_t* imin, size_t* imax ) const {
      gsl_vector_long_minmax_index( get(), imin, imax ); }
    /**
     * C++ version of gsl_vector_long_add().
     * @param b vector_long to add to @c this
     * @return error code on failure
     */
    int add( vector_long const& b ){ return gsl_vector_long_add( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_long_sub().
     * @param b vector_long to subtract from @c this
     * @return error code on failure
     */
    int sub( vector_long const& b ){ return gsl_vector_long_sub( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_long_mul().
     * @param b vector_long to multiply elementwise with @c this
     * @return error code on failure
     */
    int mul( vector_long const& b ){ return gsl_vector_long_mul( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_long_div().
     * @param b vector_long to dividev @c this by elementwise
     * @return error code on failure
     */
    int div( vector_long const& b ){ return gsl_vector_long_div( get(), b.get() ); }
    /**
     * C++ version of gsl_vector_long_scale().
     * @param x constant to multiply @c this by
     * @return error code on failure
     */
    int scale( long const x ){ return gsl_vector_long_scale( get(), x ); }
    /**
     * C++ version of gsl_vector_long_add_constant().
     * @param x constant to add to each element of @c this
     * @return error code on failure
     */
    int add_constant( long const x ){ return gsl_vector_long_add_constant( get(), x ); }
    /**
     * C++ version of gsl_vector_long_isnull().
     * @return @c +1 or @c 0 according as elements are all zero or not
     */
    int isnull() const { return gsl_vector_long_isnull( get() ); }
    /**
     * C++ version of gsl_vector_long_ispos().
     * @return @c +1 or @c 0 according as elements are all positive or not
     */
    int ispos() const { return gsl_vector_long_ispos( get() ); }
    /**
     * C++ version of gsl_vector_long_isneg().
     * @return @c +1 or @c 0 according as elements are all negative or not
     */
    int isneg() const { return gsl_vector_long_isneg( get() ); }
    /**
     * C++ version of gsl_vector_long_isnonneg().
     * @return @c +1 or @c 0 according as elements are all nonnegative or not
     */
    int isnonneg() const { return gsl_vector_long_isnonneg( get() ); }
    /**
     * C++ version of gsl_vector_long_get().
     * @param i index of element to get
     * @return value of element
     */
    long get( size_t const i ) const { return gsl_vector_long_get( get(), i ); }
    /**
     * C++ version of gsl_vector_long_set().
     * @param i index to set
     * @param x new value for element
     */
    void set( size_t const i, long x ){ gsl_vector_long_set( get(), i, x ); }
    /**
     * C++ version of gsl_vector_long_ptr().
     * @param i index of element to get
     * @return pointer to element
     */
    long* ptr( size_t const i ){ return gsl_vector_long_ptr( get(), i ); }
    /**
     * C++ version of gsl_vector_long_const_ptr().
     * @param i index of element to get
     * @return pointer to element
     */
    long const* const_ptr( size_t const i ) const { return gsl_vector_long_const_ptr( get(), i ); }
    /**
     * C++ version of gsl_vector_long_fread().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fread( FILE* stream ){ return gsl_vector_long_fread( stream, get() ); }
    /**
     * C++ version of gsl_vector_long_fwrite().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fwrite( FILE* stream ) const { return gsl_vector_long_fwrite( stream, get() ); }
    /**
     * C++ version of gsl_vector_long_fscanf().
     * @param stream A C file stream
     * @return error code on failure
     */
    int fscanf( FILE* stream ){ return gsl_vector_long_fscanf( stream, get() ); }
    /**
    ...
    int fprintf( FILE* stream, char const* format ) const {
      return gsl_vector_long_fprintf( stream, get(), format ); }
    /**
     ...
     */
    vector_long( block_long& b, size_t const offset, size_t const n, size_t const stride ){
      ccgsl_pointer = gsl_vector_long_alloc_from_block( b.get(), offset, n, stride );
      // just plausibly we could allocate vector_long but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_long_free( ccgsl_pointer );
     ...
     * @param stride The stride
     */
    vector_long( vector_long& v, size_t const offset, size_t const n, size_t const stride ){
      ccgsl_pointer = gsl_vector_long_alloc_from_vector( v.get(), offset, n, stride );
      // just plausibly we could allocate vector_long but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_vector_long_free( ccgsl_pointer );
    ...
    static vector_long view_array( long* v, size_t n ){
      ...
      *w = gsl_vector_long_view_array( v, n ).vector;
    ...
    static vector_long view_array_with_stride( long* base, size_t stride, size_t n ){
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_view_array_with_stride( base, stride, n ).vector;
      return vector_long( w );
    }
    /**
     * C++ version of gsl_vector_long_const_view_array().
     * @param v An array of type long
     * @param n The size of the vector_long
     * @return A vector_long
     */
    static vector_long const const_view_array( long const* v, size_t n ){
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_view_array( v, n ).vector;
      return vector_long( w );
    }
    /**
    ...
    static vector_long const const_view_array_with_stride( long const* base, size_t stride, size_t n ){
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_view_array_with_stride( base, stride, n ).vector;
      return vector_long( w );
    }
    /**
    ...
    vector_long subvector( size_t i, size_t n ){
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_subvector( get(), i, n ).vector;
      return vector_long( w );
    }
    /**
    ...
    vector_long subvector_with_stride( size_t i, size_t stride, size_t n ){
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_long( w );
    }
    /**
     * C++ version of gsl_vector_long_subvector_with_stride().
    ...
    vector_long const const_subvector( size_t i, size_t n ) const {
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_subvector( get(), i, n ).vector;
      return vector_long( w );
    }
    /**
     * C++ version of gsl_vector_long_subvector_with_stride().
    ...
    vector_long const const_subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_long( w );
    }
    // Extra allocators from matrix_long objects. The definition must come after matrix_long.
    /**
     * C++ version of gsl_vector_long_alloc_row_from_matrix().
     * @param m A matrix_long
     * @param i A row
     * @return A vector_long
     */
   ...
   * @see vector_long::iterator::operator+()
   */

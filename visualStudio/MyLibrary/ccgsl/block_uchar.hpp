...
#ifndef CCGSL_BLOCK_UCHAR_HPP
#define CCGSL_BLOCK_UCHAR_HPP
...
namespace gsl {
  /**
   * This class handles vector_uchars as shared handles. It models a random access container
   * so that STL functions work with block_uchar.
   */
  class block_uchar {
  public:
    /**
    ...
    // Refines assignable
    /**
     * The default constructor creates a new block_uchar with n elements
     * @param n The number of elements in the block_uchar
     */
    explicit block_uchar( size_t const n ){
      ccgsl_pointer = gsl_block_uchar_alloc( n );
      // just plausibly we could allocate block_uchar but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	gsl_block_uchar_free( ccgsl_pointer );
	throw e;
      }
      *count = 1; // initially there is just one reference to ccgsl_pointer
    }
    /**
     * Could construct from a gsl_block_uchar. This is not usually a good idea. In this case
     * we should not use gsl_block_uchar_free() to deallocate the memory.
     * @param v The block_uchar
     */
    explicit block_uchar( gsl_block_uchar* v ){
      ccgsl_pointer = v;
    ...
    // copy constructor
    /**
     * The copy constructor. This shares the block_uchar. Use clone() if you want a full copy.
     * @param v The block_uchar to copy.
     */
    block_uchar( block_uchar const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
     ...
     * @param v The block_uchar to copy
     */
    block_uchar& operator=( block_uchar const& v ){
      // first, possibly delete anything pointed to by this
      if( --*count == 0 ){
	if( ccgsl_pointer != 0 ) gsl_block_uchar_free( ccgsl_pointer );
	delete count;
      }
      // Then copy
     ...
     */
    block_uchar clone() const {
      block_uchar copy( ccgsl_pointer->size );
      // Now copy
      memcpy( copy.ccgsl_pointer->data, ccgsl_pointer->data, ccgsl_pointer->size * sizeof( unsigned char ) );
      // return new object
      return copy;
    }
    // destructor
    ...
    // == operator
    /**
     * Two block_uchar objects are identically equal if their elements are identical.
     * @param v The block_uchar to be compared with @c this
     * @return @c true or @c false according as @c this and @c v have
     * identical elements or not
     */
    bool operator==( block_uchar const& v ) const {
      // trivially equal if gsl_block_uchar*s are identical
    ...
    // != operator
    /**
     * Two block_uchar objects are different equal if their elements are not identical.
     * @param v The block_uchar to be compared with @c this
     * @return @c false or @c true according as @c this and @c v have
     * identical elements or not
     */
    bool operator!=( block_uchar const& v ) const { return not operator==( v ); }
     ...
     * for example, for checking, that a block_uchar is nonnegative.
     * @param v The block_uchar to be compared with @c this
     * @return @c false or @c true according as @c this is less than @c v
     * lexicographically
     */
    bool operator<( block_uchar const& v ) const {
      // null block_uchar comes first
	...
	unsigned char const t = ccgsl_pointer->data[i];
	unsigned char const u =v.ccgsl_pointer->data[i];
	if( t < u ) return true;
	if( u < t ) return false;
      }
      // elements match.
  ...
  private:
    ...
    class iterator_base
      ...
      friend class block_uchar;
    public:
      // // type iterator_traits<block_uchar>::difference_type
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_uchar not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
  	...
  	// Warn if iterators do not point to same block_uchar
  	if( v->ccgsl_pointer != i.v->ccgsl_pointer ){
  	  gsl_error( "trying to take difference of iterators for different block_uchars", __FILE__, __LINE__,
	...
	return not this->operator==( i );
       ...
       * if the iterators iterate over the same block_uchar and the function calls
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a block_uchar
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
      ...
      /**
       * This constructor allows block_uchar to create non-default iterators.
       * @param v The block_uchar that creates @c this
      ...
      /**
       * Store a pointer to a block_uchar we can iterate over: 0 if no block_uchar.
       */
      container* v;
      /**
       * Mark position of iterator within block_uchar.
       */
     ...
     */
    template<bool reverse> class iterator_t : public iterator_base<block_uchar,unsigned char,reverse>{
    public:
  	...
  	iterator_base<block_uchar,unsigned char,reverse>::v = i.v;
  	iterator_base<block_uchar,unsigned char,reverse>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<block_uchar,unsigned char,reverse>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse>& operator--(){
  	iterator_base<block_uchar,unsigned char,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_uchar,unsigned char,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
  	...
  	return iterator_base<block_uchar,unsigned char,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       ...
       */
      iterator_t<reverse>() : iterator_base<block_uchar,unsigned char,reverse>(){}
    protected:
      friend class block_uchar;
      // We need a constructor for block_uchar
      /**
       * This constructor allows block_uchar to create non-default iterators.
       * @param v The block_uchar that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse>( block_uchar* v, difference_type position )
      : iterator_base<block_uchar,unsigned char,reverse>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse> class const_iterator_t
      : public iterator_base<block_uchar const,unsigned char,reverse>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<block_uchar const,unsigned char,reverse>::v = i.v;
  	iterator_base<block_uchar const,unsigned char,reverse>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<block_uchar const,unsigned char,reverse>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse>& operator--(){
  	iterator_base<block_uchar const,unsigned char,reverse>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * Difference type.
       */
      typedef typename iterator_base<block_uchar const,unsigned char,reverse>::difference_type
  	difference_type;
      // // operator+=
      /**
       * The += operator.
       ...
       */
      difference_type operator-( const_iterator_t<reverse> const& i ) const {
  	return iterator_base<block_uchar const,unsigned char,reverse>::operator-( i );
      }
      /**
       * The - operator: find distance between two iterators
       * @param i A second iterator
       ...
       * The default constructor.
       */
      const_iterator_t<reverse>() : iterator_base<block_uchar,unsigned char,reverse>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse>( block_uchar const* v, difference_type position )
      : iterator_base<block_uchar const,unsigned char,reverse>( v, position ){}
    };
  public:
    /**
    ...
    // begin()
    /**
     * Get iterator pointing to first block_uchar element.
     * @return iterator pointing to first block_uchar element
     */
    iterator begin(){
    ...
    // end()
    /**
     * Get iterator pointing beyond last block_uchar element.
     * @return iterator pointing beyond last block_uchar element
     */
    iterator end(){
    ...
    // size()
    /**
     * The size (number of elements) of the block_uchar.
     * @return The size of the block_uchar
     */
    size_type size() const { return ccgsl_pointer->size; }
    // max_size()
    /**
     * The max size (number of elements) of the block_uchar. Identical to
    ...
    // empty()
    /**
     * Find if the block_uchar is empty.
    ...
    /**
     * Swap two block_uchar objects. This works even if the block_uchar objects have
     * different sizes because it swaps pointers.
     * @param v The block_uchar to swap with @c this.
     */
    void swap( block_uchar& v ){
      gsl_block_uchar* tmp = ccgsl_pointer; ccgsl_pointer = v.ccgsl_pointer; v.ccgsl_pointer = tmp;
    ...
    // rend()
    /**
     * Get iterator pointing beyon last block_uchar element.
     * @return iterator pointing beyond last block_uchar element
     */
    ...
    unsigned char& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_uchar is null", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_uchar", __FILE__, __LINE__, exception::GSL_EFAILED );
  	return something;
      }
  ...
  public:
    ...
    /**
     * Get the gsl_block_uchar.
     * @return the gsl_block_uchar
     */
    gsl_block_uchar* get() const { return ccgsl_pointer; }
    /**
     * Find if this is the only object sharing the gsl_block_uchar.
     * @return @c true or @c falses according as 
     * this is the only block_uchar object sharing the gsl_block_uchar
     */
    bool unique() const { return *count == 1; }
    /**
     * Find how many block_uchar objects share this pointer.
     * @return the number of block_uchar objects that share this pointer
     */
    size_t use_count() const { return *count; }
    /**
     * Allow conversion to bool.
     * @return @c true or @c false according as this contains a pointer
     * to a gsl_block_uchar
     */
   ...
   * @see block_uchar::iterator::operator+()
   */
  inline block_uchar::iterator operator+
  ( block_uchar::iterator::difference_type const n,
   ...
   * @see block_uchar::iterator::operator+()
   */
  inline block_uchar::const_iterator operator+
  ( block_uchar::const_iterator::difference_type const n,
  				    block_uchar::const_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_uchar::iterator::operator+()
   */
  inline block_uchar::reverse_iterator operator+
  ( block_uchar::reverse_iterator::difference_type const n,
  			      block_uchar::reverse_iterator const& i ){
    return i + n;
  }
   ...
   * @see block_uchar::iterator::operator+()
   */
  inline block_uchar::const_reverse_iterator operator+
  ( block_uchar::const_reverse_iterator::difference_type const n,
  			      block_uchar::const_reverse_iterator const& i ){
    return i + n;
  }

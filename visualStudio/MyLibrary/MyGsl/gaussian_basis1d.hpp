#ifndef __MYGSL_GAUSSIAN_BASIS1D_HPP__
#define __MYGSL_GAUSSIAN_BASIS1D_HPP__
...
namespace mygsl{
	...
	class Gaussian1d;	//	just a definition 
	class Overlap1d;
//-----------------------------------------------------------------------//
//																		 //
...
//-----------------------------------------------------------------------//
	...
	public:
		//	constructors	
		OneGaussian1d(void){ alpha=0.0; coef=0.0;}
		~OneGaussian1d(void){};
		OneGaussian1d(const OneGaussian1d& ob);
		OneGaussian1d& operator = (const OneGaussian1d& obj);

		//	overriden operators
		double operator * (const OneGaussian1d& obj)const;
		double operator * (const Gaussian1d& obj)const;
		bool operator == (const OneGaussian1d& obj)const;
		bool operator != (const OneGaussian1d& obj)const;
	...
	private:
		...
		size_t xi;				//	power index for the x, y, z
		double area;
		double moment;
		void calculateCoef();
	...
	public:
		...
		//	returns the value of <phi1| x^i |phi2>
		static double OneIntegral(  const OneGaussian1d& phi1, const OneGaussian1d& phi2, const size_t pow_x);

		//	returns the value of integ[0,  inf]{phi1| x^i |phi2}
		static double OneIntegral_0_inf(  const OneGaussian1d& phi1, const OneGaussian1d& phi2, const size_t pow_x);

		//	returns the value of integ[-inf, 0]{phi1| x^i |phi2}
		static double OneIntegral_inf_0(  const OneGaussian1d& phi1, const OneGaussian1d& phi2, const size_t pow_x);
	};

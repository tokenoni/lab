namespace mygsl{
	...
	inline Multimin::Multimin
		( TyF_& function, std::vector<double>& params, std::vector<double>& params_step, double relative_err, IsPrint isprint_)
	{
		isprint = isprint_;
	...
	template< class TyF_ >
	inline Multimin::Multimin
		( TyF_& function, std::vector<double>& params, double step_ratio, double relative_err, IsPrint isprint_)
	{
		isprint = isprint_;
		params_name.resize(0);
	...
	template<class TyF_ >
	inline Multimin::Multimin
		( TyF_& function, std::vector<double>& params, std::vector<double>& params_step, double relative_err, std::vector<std::string> params_name_, IsPrint isprint_)
	{
		isprint = isprint_;
	...
	template<class TyF_ >
	inline Multimin::Multimin
		( TyF_& function, std::vector<double>& params, double step_ratio, double relative_err, std::vector<std::string> params_name_, IsPrint isprint_)
	{
		isprint = isprint_;
		std::vector<double> params_step(params.size());
		MAXiter = 1000;
	...
	( TyF_& function, std::vector<double>& params, std::vector<double>& params_step, double relative_err)
	{
		...
		FunctionAndParameters< TyF_ >  func_and_params(&function, params.size());
		minex_func.f = &FunctionAndParameters< TyF_ >::call_back;	//cfunc.adopt();

		minex_func.n = params.size();
		minex_func.params = &func_and_params;
		...
		do{
			...
			if(isprint == Print) print_state(iter, s, size);

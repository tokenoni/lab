...
inline FFT2base& FFT2base::copy(const FFT2base& obj){
	if(obj.IsExist()){
		MemoryControl(obj.Size());
		for(size_t i=0; i<obj.Size(); ++i) f[i] = obj.f[i];
	}
...
inline std::vector<double> FFT2base::power()const{
...
inline std::vector<double> FFT2base::amplitude()const{
	std::vector<double> result(size/2+1);
	result[0] = f[0];
	for(size_t i=1; i<size/2; ++i)
...
inline std::vector<double> FFT2base::phase()const{
	std::vector<double> result(size/2+1);
	result[0] = 0.0;
	for(size_t i=1; i<size/2; ++i){
...
//											 //
//-------------------------------------------//
inline FFT2::FFT2(const size_t size_, const double rate_, FFT2::FFTwindow window_)
	: FFT2base()
{
	MemoryControl(size_);
	rate = rate_;
...
inline FFT2::FFT2(const FFT2& obj)
	: FFT2base(obj)
{
	rate = obj.Rate();
...
inline FFT2& FFT2::operator = (const FFT2& obj)
{
	(FFT2base)copy(obj);
	rate = obj.Rate();
	window = obj.getWindow();
...
//---	Power Spectrum	---
template< class Tyv_ >
inline void FFT2::Power(Tyv_& power)const{
	double T = rate/size;
	power[0] = f[0]*f[0];
	for(size_t i=1; i<Size()/2; ++i)
...
inline std::vector<double> FFT2::Power()const{
	std::vector<double> amp(Size()/2+1);
	Power(amp);
	return amp;
}
inline std::vector<double> FFT2::Phase()const{
}
//---	Cross Power Spectrum	---
template< class Tyv_ >
inline void FFT2::CrossPower(Tyv_& crosspower, const FFT2& src)const{
	double T = rate/size;
...
//---	Coherency	---
template< class Tyv_ >
inline void FFT2::Coherency(Tyv_& coherency, FFT2& src)const{
	double T = rate/size;
	coherency[0] = 1.0;
	double real, amp;
	for(size_t i=1; i<size/2; ++i){
...
//---	CrossPhase	---
template< class Tyv_ >
inline void FFT2::CrossPhase(Tyv_& crossphase, const FFT2& src)const{
	double T = rate/size;
	crossphase[0] = 0.0;
	double real, imag;
	for(size_t i=1; i<size/2; ++i){
...
template < class Tyv_ >
inline void FFT2::set(const Tyv_& raw_data, size_t size_, double rate_, FFTwindow window){
	set(raw_data, size_, 0, rate_, window);
}
template < class Tyv_ >
inline void FFT2::set(const Tyv_& raw_data, size_t size_, size_t offset, double rate_, FFTwindow window){
	rate = rate_;
...
inline void FFT2::setEachpreparation(const size_t size_, double rate_, FFTwindow window_){
	rate = rate_;
	size_t size_tmp = getPower2NumFloor(size_);
	MemoryControl(size_tmp);
	window = window_;
}
inline void FFT2::setEach(const size_t index, const double data_i){
	f[index] = data_i;
}
inline void FFT2::setEachFinish(){
	gsl_fft_real_radix2_transform(f, 1, size);
	for(size_t i=0; i<size; ++i) f[i] *= rate;
}
...
(const FFT2development& src, size_t avgnum)
{
...
inline std::vector<std::vector<double>> FFT2development::getCrossPhase(const FFT2development& src, size_t avgnum_){
	avgnum = avgnum_;

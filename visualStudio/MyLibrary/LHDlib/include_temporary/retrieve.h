...
RETRIEVE_API int retrieveGetDTSinfox
					(const char *DiagName,						// [in] 計測名
					 unsigned int AShotNumber,					// [in] 問い合わせショット番号
					 short ASubShotNumber,						// [in] 問い合わせサブショット番号
					 short Fastch,								// [in] 最始チャネル番号
					 short Lastch,								// [in] 最終チャネル番号
					 const char *IndexServer,					// [in] インデックスサーバ
...
//----------------------------------------------------------------------------------------------- 
//
// これ以降のモジュールは旧バージョン互換のためのモジュールである。
// 今後、使用しないように検討すること。
//
//----------------------------------------------------------------------------------------------- 
 ...
 *          BUG-FIX: フレーム構造のサブショットデータが取得できない。
 *
 */

...
#define fordbsopenframe2_( x1,x2,x3,x4,x5,x6 )	__stdcall FORDBSOPENFRAME2( x1,x2,x3,x4,x5,x6 )	// add 2012-04-20
...
DBSTORE_API long fordbsopenframe2_(long dbs, long ch_no,
			unsigned long  x_size,	 unsigned long  y_size, char *data_type, int sw_accept_sizes );	// add 2014-04-20
...
DBSTORE_API int dbsCloseFrame3(int dbs_d, int ch_no, const char *params_serial);	// add 2012-08-14
DBSTORE_API int dbsCloseFrameLV(int dbs_d, int ch_no, const char *params_string, int params_len);	// add 2013-06-10
...
DBSTORE_API int dbsWrite6(int dbs_d, int ch_no,
			const char* params_serial,
			unsigned int data_length, void* data, char* data_type );
DBSTORE_API int dbsWriteLV(int dbs_d, int ch_no,
			int params_len, const char* params_buffer, 
			unsigned int data_length, void* data, char* data_type );

DBSTORE_API int dbsOpen2(const char *mail_address, const char *diag_name, 
			 unsigned int shot_number, unsigned short sub_shot, short data_type,
			 int connect_db_timeout, int connect_db_retry);

DBSTORE_API int dbsOpenFrame2(int dbs_d, int ch_no,
			unsigned int x_size,unsigned int y_size, char* data_type, int sw_accept_sizes );	// add 2010-04-20
 ...
 * @note	none update
 */
#define DBSTORE_MAJOR	17
#define DBSTORE_MINOR	0
#ifndef DBSTORE_TINY
 #define DBSTORE_TINY	0
#endif
#define DBSTORE_VERSION	DSTR(DBSTORE_MAJOR) "." DSTR(DBSTORE_MINOR) "." DSTR(DBSTORE_TINY)
/***************************************************
 parameter type
****************************************************/
#define TYPE_UNDEF	0

...
//---	get the interpolated value at x_interp	---
	...
	double operator() (double x_interp){ return get(x_interp);}
...
public:	
	...
	void df_multiroot( const gsl_vector *coef_, gsl_vector* f);
	EdgeLogSplineArea(void){
			T = const_cast<gsl_multiroot_fsolver_type*> (gsl_multiroot_fsolver_hybrids);
	};

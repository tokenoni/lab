...
inline void EdgeSpline::set(const TyE_& y_src, size_t size_, EndPoint endpoint_){
	...
	endpoint = endpoint_;
	sort();
	for(size_t i=1; i<size; ++i) x_edge[i-1] = 0.5*(x[i-1]+x[i]);
	setScale();
	solve();
}
template < class TyE_ >
...
template < class TyE_ >
inline void EdgeSpline::set(const TyE_& x_src, const TyE_& y_src, double dx_, size_t size_){
	set(x_src, y_src, dx_, size_, Free);
}
template < class TyE_ >
inline void EdgeSpline::set(const TyE_& x_src, const TyE_& y_src, double dx_, size_t size_, EndPoint endpoint_){
	endpoint = endpoint_;
	MemoryControl(size_);	
	for(size_t i=0; i<size; ++i){
		x[i] = x_src[i];	y[i] = y_src[i];		dx[i]= dx_;
...
inline void EdgeSpline::setScale(){ scale = Linear;}
	
inline double EdgeSpline::get(double x_interp){
...
inline void EdgeSpline::sort(){
	for(size_t i=0; i<size; ++i)
		gsl_vector_set(v_sort, i, x[i]);
	gsl_sort_vector_index(p_sort, v_sort);
	
	for(size_t i=0; i<size; ++i)
		x[i] = gsl_vector_get(v_sort, gsl_permutation_get(p_sort, i));
	for(size_t i=0; i<size; ++i)
		gsl_vector_set(v_sort, i, y[i]);
	for(size_t i=0; i<size; ++i)
		y[i] = gsl_vector_get(v_sort, gsl_permutation_get(p_sort, i));
}
...
inline void EdgeSpline::Allocation(){
	...
	v_sort = gsl_vector_calloc(size);
	p_sort = gsl_permutation_calloc(size);
	Allocated = true;
}
inline void EdgeSpline::FreeMemory(){
	...
	gsl_permutation_free(p_sort);
	gsl_vector_free(v_sort);
	Allocated = false;
}
...
inline void EdgeLogSplineArea::f_multiroot(const gsl_vector *coef_, gsl_vector *f){

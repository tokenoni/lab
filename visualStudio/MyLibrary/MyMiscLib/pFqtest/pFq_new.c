/* ../fortran_sources/pFq.new.f -- translated by f2c (version 20090411).
...
/*    extern  Double Complex  VOID hyper_(doublecomplex *, doublecomplex *, 
 ...
 /*   extern Double Complex VOID cgamma_(doublecomplex *, doublecomplex *,
    ...
    if (*lnpfq != 0 && *lnpfq != 1) {
		std::cerr<< " ERROR IN INPUT ARGUMENTS: LNPFQ /= 0 OR 1" << std::endl;
		getchar();
		exit(8);
/*	io___1.ciunit = io_1.nout;
  ...
  */  }
    if (*ip > *iq && z_abs(z__) > consts_1.one && *ip > 2) {
		std::cerr<< " ip=" << *ip <<", iq="<<*iq<< ", abs(z)"<<z_abs(z__)<<std::endl;
...
/* For the 2F1 function with |z| > 1, use Abramowitz and Stegun, 15.3.7. */
	...
	z__2.r = consts_1.one, z__2.i = 0.;	//	ONE in pFq.new.f line 105
	z_div(&z__1, &z__2, z__);

...
inline bool ParameterFileImport::readFile(const std::string filename_){
	...
	if (!ifs.good()){
		std::string errormessage = "Error in ParameterFileImport::readFile\n" 
			"filename:" +  filename + " is not found\n";
		throw errormessage;
	}
...
inline size_t ParameterFileImport::searchIndex(const std::string parameter_name)const
{
	...
	if(i==parameter_names.size()){
		std::string errormessage = std::string("Error in ParameterFileImport::searchIndex\n")
			+ "parameter name " + parameter_name + " is not found in " + filename;
		throw errormessage;
	}
	return i;
}
...
inline std::vector<std::string> ParameterFileImport::getValueList_string(const std::string paraname)const{
	...
	if (rslt.size() == 0){
		std::string errormessage = std::string("Error in ParameterFileImport::getValueList_string\n")
			+ "parameter name " + paraname + " is not found in " + filename;
		throw errormessage;
	}
	return rslt;
}

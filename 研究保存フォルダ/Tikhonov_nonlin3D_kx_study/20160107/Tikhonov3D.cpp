...
/*
	...
	for (size_t trial = 0; trial < trialmax; ++trial){
		if (updateFandVandLam(relativeError0, vError1, lam1Error, lam2Error)){
			count(4) = 1.0*trial;
#ifdef DEBUG
			IGORdata::write_itx(count, "output/prac/count.itx", "count");
			IGORdata::write_itx(count, "output/debug/count.itx", "count");
...
bool Tikhonov3D::updateFandVandLam
(const double relativeError0, const double vError1,
 const double lam1Error, const double lam2Error){
...
///Ç±Ç±Ç‹Ç≈Ç®Çã
	...
	for (size_t i = 0; i < size_i; ++i){
		...
		for (size_t l = 0; l < size_j; ++l){
			...
			Hxk -= res_i * Adx_zee(i, l, k, x)   * z[l] / sigma[i] / sigma[i];
			Hxx -= res_i * Adxdx_zee(i, l, k, x) * z[l] / sigma[i] / sigma[i];
			Hkk += res_i * Adkdk_zee(i, l, k, x) * z[l] / sigma[i] / sigma[i];

			for (size_t ll = 0; ll < size_j; ++ll){
				Hxk += Adk_zee(i, l, k, x) * z[l] * Adx_zee(i, ll, k, x) * z[ll] / sigma[i] / sigma[i];
				Hxx -= Adx_zee(i, l, k, x) * z[l] * Adx_zee(i, ll, k, x) * z[ll] / sigma[i] / sigma[i];
				Hkk += Adk_zee(i, l, k, x) * z[l] * Adk_zee(i, ll, k, x) * z[ll] / sigma[i] / sigma[i];
			}
		}
		...
		for (size_t j = 0; j < size_j; ++j){
			...
			Hfk[j] -= res_i * Adk_zee(i, j, k, x)   * z[j] / sigma[i] / sigma[i];
			Hvk[j] -= res_i * Adkdv_zee(i, j, k, x) * z[j] / sigma[i] / sigma[i];
			Hfx[j] -= res_i * Adx_zee(i, j, k, x)   * z[j] / sigma[i] / sigma[i];
			Hvx[j] -= res_i * Adx_zee(i, j, k, x)   * z[j] / sigma[i] / sigma[i];

			for (size_t l = 0; l < size_j; ++l){
				Hfk[j] += Adk_zee(i, l, k, x)   * z[l] * A_zee(i, j, k, x)  * z[j] / sigma[i] / sigma[i];
				Hvk[j] += Adk_zee(i, l, k, x)   * z[l] * Adv_zee(i, j, k, x)* z[j] / sigma[i] / sigma[i];
				Hfx[j] += Adx_zee(i, l, k, x)   * z[l] * A_zee(i, j, k, x)  * z[j] / sigma[i] / sigma[i];
				Hvx[j] += Adxdv_zee(i, l, k, x) * z[l] * Adv_zee(i, j, k, x)* z[j] / sigma[i] / sigma[i];
			}
	...
	H.block(2*size_j+2,          0,      1, size_j) = Hfk;
	H.block(2*size_j+3,          0,      1, size_j) = Hfx;
	...
	H.block(2*size_j+2,     size_j,      1, size_j) = Hvk;
	H.block(2*size_j+3,     size_j,      1, size_j) = Hvx;
	H.block(         0,   2*size_j, size_j,      1) = H1f;
	H.block(    size_j, 2*size_j+1, size_j,      1) = H2v;
	H.block(         0, 2*size_j+2, size_j,      1) = Hfk.transpose(); //Hkf
	H.block(    size_j, 2*size_j+2, size_j,      1) = Hvk.transpose(); //Hkv
	H.block(         0, 2*size_j+3, size_j,      1) = Hfx.transpose(); //Hxf
	H.block    (size_j, 2*size_j+3, size_j,      1) = Hvx.transpose(); //Hxv

	H(  2*size_j,   2*size_j) = H11;
	H(2*size_j+1, 2*size_j+1) = H22;
	H(2*size_j+2, 2*size_j+2) = Hkk;
	H(2*size_j+3, 2*size_j+2) = Hxk; // Hkx
	H(2*size_j+2, 2*size_j+3) = Hxk;
	H(2*size_j+3, 2*size_j+3) = Hxx;

	return true;
}

double Tikhonov3D::A_zee(const size_t i, const size_t j, const double k, const double x)const{
	return (1 - 1 / (2 + exp(k)))*A_(i, j, v[j]) + 0.5 / (2 + exp(k))*A_(i, j, v[j] - x) + 0.5 / (2 + exp(k))*A_(i, j, v[j] + x);
}

double Tikhonov3D::Adv_zee(const size_t i, const size_t j, const double k, const double x)const{
	return (1 - 1 / (2 + exp(k)))*Adv_(i, j, v[j]) + 0.5 / (2 + exp(k)) * Adv_(i, j, v[j] - x) + 0.5 / (2 + exp(k)) * Adv_(i, j, v[j] + x);
}
...
double Tikhonov3D::Adx_zee(const size_t i, const size_t j, const double k, const double x)const{
	return -0.5 / (2 + exp(k)) * Adv_(i, j, v[j] - x) + 0.5 / (2 + exp(k))*Adv_(i, j, v[j] + x);
}

double Tikhonov3D::Adxdv_zee(const size_t i, const size_t j, const double k, const double x)const{
	return -0.5 / (2 + exp(k)) * Advdv_(i, j, v[j] - x) + 0.5 / (2 + exp(k))*Advdv_(i, j, v[j] + x);
}
...
double Tikhonov3D::A_(const size_t i, const size_t j, const double velocity)const{
	...
	if (j == size_j - 1)
		...
		return (exp(-((wavelength(i) - lambda0 + velocity * lambda0 / vlight) / width(j)) 
	        	* ((wavelength(i) - lambda0 + velocity * lambda0 / vlight) / width(j))));

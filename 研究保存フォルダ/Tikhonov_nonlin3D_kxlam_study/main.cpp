...
int main(){
	...
	for (size_t ki = 0; ki < 5; ki += 1){
		for (size_t xi = 0; xi < 5; xi += 1){
			std::string ki_str = std::to_string(ki);
			std::string xi_str = std::to_string(xi);
			...
			double x0 = 100 + 1000 * xi;
			double k0 = log(0.1 + ki);
			f0.setOnes();
			f0 *= 5.0;
			v0.setOnes();
			v0 *= 500;
			...
			tikhonov.solve(1.0e-2, 1, 0.01, 0.01, 0.1, 1);
			//	tikhonov.solve(1.0e-3, 0.1, 0.001, 0.001, 0.01, 0.1);


			IGORdata::write_itx(tikhonov.getLam(), "output/prac/"+xi_str+"/"+ki_str+"/lam.itx", "lam");
			IGORdata::write_itx(tikhonov.getA(), "output/prac/" + xi_str + "/" + ki_str + "/A.itx", "AA");
			IGORdata::write_itx(tikhonov.getF(), "output/prac/" + xi_str + "/" + ki_str + "/f.itx", "FF");
			IGORdata::write_itx(tikhonov.getV(), "output/prac/" + xi_str + "/" + ki_str + "/v.itx", "vv");
			IGORdata::write_itx(tikhonov.getAf(), "output/prac/" + xi_str + "/" + ki_str + "/Af.itx", "Af");
			IGORdata::write_itx(tikhonov.getRes_sigma(), "output/prac/" + xi_str + "/" + ki_str + "/res_sigma.itx", "res_sigma");
			IGORdata::write_itx(tikhonov.getRes(), "output/prac/" + xi_str + "/" + ki_str + "/res.itx", "res");
			IGORdata::write_itx(tikhonov.getDf(), "output/prac/" + xi_str + "/" + ki_str + "/df.itx", "df");
			IGORdata::write_itx(tikhonov.getDv(), "output/prac/" + xi_str + "/" + ki_str + "/dv.itx", "dv");
			IGORdata::write_itx(tikhonov.getZ(), "output/prac/" + xi_str + "/" + ki_str + "/z.itx", "ZZ");
			IGORdata::write_itx(tikhonov.getKX(), "output/prac/" + xi_str + "/" + ki_str + "/knx.itx", "KX");
			IGORdata::write_itx(tikhonov.getCenterAf(), "output/prac/" + xi_str + "/" + ki_str + "/centerAf.itx", "centerAf");
			IGORdata::write_itx(tikhonov.getLeftAf(), "output/prac/" + xi_str + "/" + ki_str + "/leftAf.itx", "leftAf");
			IGORdata::write_itx(tikhonov.getRightAf(), "output/prac/" + xi_str + "/" + ki_str + "/rightAf.itx", "rightAf");
...
//ここから先はデータの並び替えか何かをするときに使っただけ．上とは別．
